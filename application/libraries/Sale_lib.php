...
class Sale_lib
{
  ...
  {
    return $this->CI->session->userdata('fecha_pago') ? $this->CI->session->userdata('fecha_pago') : '';
  }
  ...
  function clear_fecha_pago()   
  {
    $this->CI->session->unset_userdata('fecha_pago');
  ...
  {
    $this->CI->session->set_userdata('fecha_pago',$fecha_pago);
  }
  ...
  function get_latsale() 
  {
    return $this->CI->session->userdata('latsale') ? $this->CI->session->userdata('latsale') : '';
  }
  
  function get_longsale() 
  {
    return $this->CI->session->userdata('longsale') ? $this->CI->session->userdata('longsale') : '';
  }
  ...
  function set_latsale($latsale) 
  {
    $this->CI->session->set_userdata('latsale', $latsale);
  }
  function set_longsale($longsale) 
  {
    $this->CI->session->set_userdata('longsale', $longsale);
  }
  ...
  function clear_latsale()   
  {
    $this->CI->session->unset_userdata('latsale');
    
  }
  
  function clear_longsale()   
  {
    $this->CI->session->unset_userdata('longsale');
  ...
  function get_payments_totals_excluding_store_account()
  {
    ...
    foreach($this->get_payments() as $payments)
    {
      ...
      {
        if ($payments['payment_type'] != lang('sales_amortizacion'))
        {
          if ($payments['payment_type'] != lang('sales_cuotainicial'))
          {
  ...
  */
  ...
  function add_item($item_id,$quantity=1,$plazo=60,$tasa=12,$frecuencia=1,$cuotainicial=0,$cuotamensual=0,$discount=0,$price=null,$description=null,$serialnumber=null, $force_add = FALSE, $line = FALSE)
  {
  

$i = $tasa;
$t = $plazo;
$p = $price - $cuotainicial;  
$f = $frecuencia;
...
$cuotamensual = $tasa1 * $valor_actual * pow((1 + $tasa1), $frecuencia1) / (1 - pow((1 + $tasa1), $frecuencia1));;
  ...
  function edit_item($line, $description = FALSE,$serialnumber = FALSE,$quantity = FALSE,$plazo = FALSE, $tasa = FALSE,$frecuencia = FALSE, $cuotainicial = FALSE, $cuotamensual = FALSE, $discount = FALSE,$price = FALSE)
  {
    $items = $this->get_cart();
    if(isset($items[$line]))
    {
      ...
      if ($plazo !== FALSE ) {
        $items[$line]['plazo'] = $plazo;
      }
      if ($tasa !== FALSE ) {
        $items[$line]['tasa'] = $tasa;
      }
      if ($frecuencia !== FALSE ) {
        $items[$line]['frecuencia'] = $frecuencia;
      }
      if ($cuotainicial!== FALSE ) {
        $items[$line]['cuotainicial'] = $cuotainicial;
      }
      if ($cuotamensual !== FALSE ) {
        $items[$line]['cuotamensual'] = $cuotamensual;
      }
      if ($price !== FALSE ) {
  ...
  function copy_entire_sale($sale_id, $is_receipt = false)
  {
    ...
    foreach($this->CI->Sale->get_sale_items($sale_id)->result() as $row)
    {
      ...
      $this->add_item($row->item_id,$row->quantity_purchased,$row->plazo,$row->tasa,$row->frecuencia,$row->cuotainicial,$row->cuotamensual,$row->discount_percent,$price_to_use,$row->description,$row->serialnumber, TRUE, $row->line);
    }
    ...
    foreach($this->CI->Sale->get_sale_item_kits($sale_id)->result() as $row)
    {
      ...
      $this->add_item_kit('KIT '.$row->item_kit_id,$row->quantity_purchased,$row->plazo,$row->tasa,$row->frecuencia,$row->cuotainicial,$row->cuotamensual,$row->discount_percent,$price_to_use,$row->description, $row->line);
    }
    ...
    $this->set_fecha_pago($this->CI->Sale->get_fecha_pago($sale_id));
    
    $this->set_latsale($this->CI->Sale->get_latsale($sale_id));
    $this->set_longsale($this->CI->Sale->get_longsale($sale_id));
  ...
  function save_current_sale_state()
  {
    $this->sale_state = array(
      ...
      'fecha_pago' => $this->get_fecha_pago(),
  ...
  function restore_current_sale_state()
  {
    if (isset($this->sale_state))
    {
      ...
      $this->set_fecha_pago($this->sale_state['fecha_pago']);
