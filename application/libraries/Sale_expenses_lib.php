...
class Sale_expenses_lib
{
  ...
  function get_cart()
  {
    if($this->CI->session->userdata('cart_expenses') === false)
      $this->set_cart(array());

    return $this->CI->session->userdata('cart_expenses');
  }

  function set_cart($cart_data)
  {
    $this->CI->session->set_userdata('cart_expenses',$cart_data);
  }

  //Alain Multiple Payments
  function get_payments()
  {
    if($this->CI->session->userdata('payments_expenses') === false)
      $this->set_payments(array());

    return $this->CI->session->userdata('payments_expenses');
  }

  //Alain Multiple Payments
  function set_payments($payments_data)
  {
    $this->CI->session->set_userdata('payments_expenses',$payments_data);
  }
  ...
  {
    return $this->CI->session->userdata('change_sale_date_expenses') ? $this->CI->session->userdata('change_sale_date_expenses') : '';
  }
  function clear_change_sale_date()   
  {
    $this->CI->session->unset_userdata('change_sale_date_expenses');
    
  }
  function clear_change_sale_date_enable()   
  {
    $this->CI->session->unset_userdata('change_sale_date_enable_expenses');
  }
  function set_change_sale_date_enable($change_sale_date_enable)
  {
    $this->CI->session->set_userdata('change_sale_date_enable_expenses',$change_sale_date_enable);
  }
  
  function get_change_sale_date_enable() 
  {
    return $this->CI->session->userdata('change_sale_date_enable_expenses') ? $this->CI->session->userdata('change_sale_date_enable_expenses') : '';
  }
  
  function set_change_sale_date($change_sale_date)
  {
    $this->CI->session->set_userdata('change_sale_date_expenses',$change_sale_date);
  }
  
  function get_comment() 
  {
    return $this->CI->session->userdata('comment_expenses') ? $this->CI->session->userdata('comment_expenses') : '';
  }
  ...
  {
    $this->CI->session->set_userdata('comment_expenses', $comment);
  }
    
  function get_selected_tier_id() 
  {
    return $this->CI->session->userdata('selected_tier_id_expenses') ? $this->CI->session->userdata('selected_tier_id_expenses') : FALSE;
  }

  function get_previous_tier_id() 
  {
    return $this->CI->session->userdata('previous_tier_id_expenses') ? $this->CI->session->userdata('previous_tier_id_expenses') : FALSE;
  }

  function set_selected_tier_id($tier_id, $change_price = true) 
  {
    $this->CI->session->set_userdata('previous_tier_id_expenses', $this->get_selected_tier_id());
    $this->CI->session->set_userdata('selected_tier_id_expenses', $tier_id);
  ...
  {
    $this->CI->session->unset_userdata('previous_tier_id_expenses');
    $this->CI->session->unset_userdata('selected_tier_id_expenses');
  }
  
  
  function set_comment_on_receipt($comment_on_receipt) 
  {
    $this->CI->session->set_userdata('show_comment_on_receipt_expenses', $comment_on_receipt);
  }

  function clear_comment()   
  {
    $this->CI->session->unset_userdata('comment_expenses');
    
  }
  
  function clear_show_comment_on_receipt()   
  {
    $this->CI->session->unset_userdata('show_comment_on_receipt_expenses');
    
  }
  
  function get_email_receipt() 
  {
    return $this->CI->session->userdata('email_receipt_expenses');
  }

  function set_email_receipt($email_receipt) 
  {
    $this->CI->session->set_userdata('email_receipt_expenses', $email_receipt);
  }

  function clear_email_receipt()   
  {
    $this->CI->session->unset_userdata('email_receipt_expenses');
  }
    
  function get_deleted_taxes() 
  {
    $deleted_taxes = $this->CI->session->userdata('deleted_taxes_expenses') ? $this->CI->session->userdata('deleted_taxes_expenses') : array();
  ...
  function add_deleted_tax($name) 
  {
    ...
    $this->CI->session->set_userdata('deleted_taxes_expenses', $deleted_taxes);
  }
  ...
  {
    $this->CI->session->unset_userdata('deleted_taxes_expenses');
  }  
  
  function get_save_credit_card_info() 
  {
    return $this->CI->session->userdata('save_credit_card_info_expenses');
  }

  function set_save_credit_card_info($save_credit_card_info) 
  {
    $this->CI->session->set_userdata('save_credit_card_info_expenses', $save_credit_card_info);
  }

  function clear_save_credit_card_info()   
  {
    $this->CI->session->unset_userdata('save_credit_card_info_expenses');
  }
  
  function get_use_saved_cc_info() 
  {
    return $this->CI->session->userdata('use_saved_cc_info_expenses');
  }

  function set_use_saved_cc_info($use_saved_cc_info) 
  {
    $this->CI->session->set_userdata('use_saved_cc_info_expenses', $use_saved_cc_info);
  }

  function clear_use_saved_cc_info()   
  {
    $this->CI->session->unset_userdata('use_saved_cc_info_expenses');
  }
  
  function get_partial_transactions()
  {
    return $this->CI->session->userdata('partial_transactions_expenses');
  }
  
  function set_partial_transactions($partial_transactions)
  {
    $this->CI->session->set_userdata('partial_transactions_expenses', $partial_transactions);
  }
  
  function add_partial_transaction($partial_transaction)
  {
    $partial_transactions = $this->CI->session->userdata('partial_transactions_expenses');
  ...
  {
    $this->CI->session->unset_userdata('partial_transactions_expenses');
  }
  ...
  function get_price_for_item($item_id, $tier_id = FALSE)
  {
    ...
    $item_info = $this->CI->Item_expenses->get_info($item_id);
    $item_location_info = $this->CI->Item_location_expenses->get_info($item_id);
    
    $item_tier_row = $this->CI->Item_expenses->get_tier_price_row($tier_id, $item_id);
    $item_location_tier_row = $this->CI->Item_location_expenses->get_tier_price_row($tier_id, $item_id, $this->CI->Employee->get_logged_in_employee_current_location_id());
  ...
  function get_price_for_item_kit($item_kit_id, $tier_id = FALSE)
  {
    ...
    $item_kit_info = $this->CI->Item_kit_expenses->get_info($item_kit_id);
    $item_kit_location_info = $this->CI->Item_kit_location_expenses->get_info($item_kit_id);
    
    $item_kit_tier_row = $this->CI->Item_kit_expenses->get_tier_price_row($tier_id, $item_kit_id);
    $item_kit_location_tier_row = $this->CI->Item_kit_location_expenses->get_tier_price_row($tier_id, $item_kit_id, $this->CI->Employee->get_logged_in_employee_current_location_id());
  ...
  {
    $this->CI->session->unset_userdata('payments_expenses');
  }
  ...
  {
    if(!$this->CI->session->userdata('customer_expenses'))
      $this->set_customer(-1, false);

    return $this->CI->session->userdata('customer_expenses');
  }
  ...
  function set_customer($customer_id, $change_price = true)
  {
    ...
    {
      $this->CI->session->set_userdata('customer_expenses',$customer_id);
  ...
  function get_mode()
  {
    if(!$this->CI->session->userdata('sale_mode_expenses'))
      $this->set_mode('sale_expenses');

    return $this->CI->session->userdata('sale_mode_expenses');
  }

  function set_mode($mode)
  {
    $this->CI->session->set_userdata('sale_mode_expenses',$mode);
  }
  ...
  */
  ...
  {
    $store_account_item_id = $this->CI->Item_expenses->get_store_account_item_id();
    ...
    if(!$this->CI->Item_expenses->exists(is_numeric($item_id) ? (int)$item_id : -1))  
    {
      //try to get item id given an item_number
      $item_id = $this->CI->Item_expenses->get_item_id($item_id);
  ...
  function add_item_kit($external_item_kit_id_or_item_number,$quantity=1,$discount=0,$price=null,$description=null, $line=FALSE)
  {
    ...
    {
      $item_kit_id = $this->CI->Item_kit_expenses->get_item_kit_id($external_item_kit_id_or_item_number);
    }
    
    
    //make sure item exists
    if(!$this->CI->Item_kit_expenses->exists($item_kit_id))  
    {
      return false;
    }
  ...
  {
    //make sure item exists
    if(!$this->CI->Item_expenses->exists($item_id))
    {
      //try to get item id given an item_number
      $item_id = $this->CI->Item_expenses->get_item_id($item_id);
    ...
    $item_location_quantity = $this->CI->Item_location_expenses->get_location_quantity($item_id);
  ...
  function out_of_stock_kit($kit_id)
  {
      ...
      if(!$this->CI->Item_kit_expenses->exists($kit_id)) return FALSE;
      ...
      {
      $item_location_quantity = $this->CI->Item_location_expenses->get_location_quantity($item->item_id);
  ...
  {    
    ...
    if(strpos(strtolower($receipt_sale_id), strtolower($this->CI->config->item('sale_prefix')).' ') !== FALSE)
    {
      ...
      return $this->CI->Sale_expenses->exists($sale_id);
    }
  ...
  function is_valid_item_kit($item_kit_id)
  {
    ...
    {
      return $this->CI->Item_kit_expenses->exists($pieces[1]);
    }
    else
    {
      return $this->CI->Item_kit_expenses->get_item_kit_id($item_kit_id) !== FALSE;
    }
  }
  ...
  function get_valid_item_kit_id($item_kit_id)
  {
    ...
    {
      return $this->CI->Item_kit_expenses->get_item_kit_id($item_kit_id);
    }
  }
  ...
  function return_entire_sale($receipt_sale_id)
  {
    ...
    foreach($this->CI->Sale_expenses->get_sale_items($sale_id)->result() as $row)
    {
      $item_info = $this->CI->Item_expenses->get_info($row->item_id);
    ...
    foreach($this->CI->Sale_expenses->get_sale_item_kits($sale_id)->result() as $row)
    {
      $item_kit_info = $this->CI->Item_kit_expenses->get_info($row->item_kit_id);
    ...
    $this->set_customer($this->CI->Sale_expenses->get_customer($sale_id)->person_id, false);
  }
  ...
  function copy_entire_sale($sale_id, $is_receipt = false)
  {
    ...
    foreach($this->CI->Sale_expenses->get_sale_payments($sale_id)->result() as $row)
    {
      $this->add_payment($row->payment_type,$row->payment_amount, $row->payment_date, $row->truncated_card, $row->card_issuer);
    }
    $customer_info = $this->CI->Sale_expenses->get_customer($sale_id);
    $this->set_customer($customer_info->person_id, false);
        
    $this->set_comment($this->CI->Sale_expenses->get_comment($sale_id));
    $this->set_comment_on_receipt($this->CI->Sale_expenses->get_comment_on_receipt($sale_id));

    $this->set_sold_by_employee_id($this->CI->Sale_expenses->get_sold_by_employee_id($sale_id));

  }

  function get_suspended_sale_id()
  {
    return $this->CI->session->userdata('suspended_sale_id_expenses');
  }
  
  function set_suspended_sale_id($suspended_sale_id)
  {
    $this->CI->session->set_userdata('suspended_sale_id_expenses',$suspended_sale_id);
  }
  
  function delete_suspended_sale_id()
  {
    $this->CI->session->unset_userdata('suspended_sale_id_expenses');
  }
  
  function get_change_sale_id()
  {
    return $this->CI->session->userdata('change_sale_id_expenses');
  }
  
  function set_change_sale_id($change_sale_id)
  {
    $this->CI->session->set_userdata('change_sale_id_expenses',$change_sale_id);
  }
  
  function delete_change_sale_id()
  {
    $this->CI->session->unset_userdata('change_sale_id_expenses');
  }
  function delete_item($line)
  {
    $items=$this->get_cart();
    $item_id=$this->get_item_id($line);
    if($this->CI->Giftcard->get_giftcard_id($this->CI->Item_expenses->get_info($item_id)->description))
    {
      $this->CI->Giftcard->delete_completely($this->CI->Item_expenses->get_info($item_id)->description);
    }
    unset($items[$line]);
    $this->set_cart($items);
  }
  ...
  function empty_cart()
  {
    $this->CI->session->unset_userdata('cart_expenses');
  }

  function delete_customer($change_price = true)
  {
    $this->CI->session->unset_userdata('customer_expenses');
  ...
  function clear_mode()
  {
    $this->CI->session->unset_userdata('sale_mode_expenses');
  }
  
  function clear_cc_info()
  {
    $this->CI->session->unset_userdata('ref_no_expenses');
    $this->CI->session->unset_userdata('auth_code_expenses');
    $this->CI->session->unset_userdata('masked_account_expenses');
    $this->CI->session->unset_userdata('card_issuer_expenses');
  }
  ...
  function restore_current_sale_state()
  {
    if (isset($this->sale_state))
    {
      ...
      $this->set_cart($this->sale_state['cart_expenses']);
  ...
  function get_taxes($sale_id = false)
  {
    ...
    if ($sale_id)
    {
      $taxes_from_sale = array_merge($this->CI->Sale_expenses->get_sale_items_taxes($sale_id), $this->CI->Sale_expenses->get_sale_item_kits_taxes($sale_id));
  ...
  function _get_price_for_item_in_cart($item, $sale_id = FALSE)
  {
    ...
    if (isset($item['item_id']))
    {
      $item_info = $this->CI->Item_expenses->get_info($item['item_id']);
    ...
    elseif (isset($item['item_kit_id']))
    {
      $item_kit_info = $this->CI->Item_kit_expenses->get_info($item['item_kit_id']);
