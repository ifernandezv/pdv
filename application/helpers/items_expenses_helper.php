...
function get_items_barcode_data($item_ids)
{
  ...
  {
    $item_info = $CI->Item_expenses->get_info($item_id);
    $item_location_info = $CI->Item_location_expenses->get_info($item_id);
...
function get_price_for_item_excluding_taxes($item_id_or_line, $item_price_including_tax, $sale_id = FALSE)
{
  ...
  if ($sale_id !== FALSE)
  {
    $tax_info = $CI->Sale_expenses->get_sale_items_taxes($sale_id, $item_id_or_line);
  }  
  else
  {
...
{
  $CI =& get_instance();
  $CI->load->library('sale_expenses_lib');

  $employee_id=$CI->sale_expenses_lib->get_sold_by_employee_id();
