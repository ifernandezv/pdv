...
function get_person_data_row($person,$controller)
{
  ...
  /* aca genero el gmaps link */
  
  $lat = $person->lat;
  $long = $person->long;
  $lat1 = $lat;
  $long1 = $long;
   if ($lat>=0) $latnordsud='N' ;
    if ($lat<0) $latnordsud='S' ;
    if ($long>=0) $lngestouest='E';
    if ($long<0) $lngestouest='W';
    $lat=abs($lat);  
    $long=abs($long);
    $latdeg=floor($lat);
    $latmin=floor(($lat-$latdeg)*60);
    $latsec=round(($lat-$latdeg-$latmin/60)*1000*3600)/1000;
    $lngdeg=floor($long);
    $lngmin=floor(($long-$lngdeg)*60);
    $lngsec=floor(($long-$lngdeg-$lngmin/60)*1000*3600)/1000;
  
  $gmaps =$latdeg."&deg;".$latmin."'".$latsec."&quot;".$latnordsud."+".$lngdeg."&deg;".$lngmin."'".$lngsec."&quot;".$lngestouest;
