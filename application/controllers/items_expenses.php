...
class Items_expenses extends Secure_area implements iData_controller
{
  function __construct()
  {
    parent::__construct('items_expenses');
  }
  ...
  {  
    ...
    {
       redirect('items_expenses/index/'.$params['offset']);
    }

    $this->check_action_permission('search');
    $config['base_url'] = site_url('items_expenses/sorting');
    ...
    if ($data['search'] || $data['category'])
    {
      ...
      $config['total_rows'] = $this->Item_expenses->search_count_all($data['search'], $data['category']);
      $table_data = $this->Item_expenses->search($data['search'],$data['category'],$data['per_page'],$params['offset'],$params['order_col'],$params['order_dir']);
    }
    else
    {
      $config['total_rows'] = $this->Item_expenses->count_all();
      $table_data = $this->Item_expenses->get_all($data['per_page'],$params['offset'],$params['order_col'],$params['order_dir']);
    }
    ...
    foreach($this->Item_expenses->get_all_categories()->result() as $category)
    {
      $category = $category->category;
      $data['categories'][$category] = $category;
    }
    
    $this->load->view('items_expenses/manage',$data);
  }
  ...
  function sorting()
  {
    ...
    if ($search || $category)
    {
      $config['total_rows'] = $this->Item_expenses->search_count_all($search, $category);
      $table_data = $this->Item_expenses->search($search,$category, $per_page,$this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'name' ,$this->input->post('order_dir') ? $this->input->post('order_dir'): 'asc');
    }
    else
    {
      $config['total_rows'] = $this->Item_expenses->count_all();
      $table_data = $this->Item_expenses->get_all($per_page,$this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'name' ,$this->input->post('order_dir') ? $this->input->post('order_dir'): 'asc');
    }
    $config['base_url'] = site_url('items_expenses/sorting');
  ...
  function find_item_info()
  {
    ...
    echo json_encode($this->Item_expenses->find_item_info($item_number));
  }
    
  function item_number_exists()
  {
    if($this->Item_expenses->account_number_exists($this->input->post('item_number')))
    echo 'false';
    else
    echo 'true';
  ...
  {
    if($this->Item_expenses->product_id_exists($this->input->post('product_id')))
    echo 'false';
    else
    echo 'true';
  ...
  {
    echo json_encode(array('duplicate'=>$this->Item_expenses->check_duplicate($this->input->post('term'))));
  }
  ...
  function search()
  {
    ...
    $this->session->set_userdata("item_search_data",$item_search_data);
    $per_page=$this->config->item('number_of_items_per_page') ? (int)$this->config->item('number_of_items_per_page') : 20;
    $search_data=$this->Item_expenses->search($search, $category, $per_page,$this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'name' ,$this->input->post('order_dir') ? $this->input->post('order_dir'): 'asc');
    $config['base_url'] = site_url('items_expenses/search');
    $config['total_rows'] = $this->Item_expenses->search_count_all($search, $category);
    $config['per_page'] = $per_page ;
    $this->pagination->initialize($config);        
    $data['pagination'] = $this->pagination->create_links();
  ...
  */
  function suggest()
  {
    ...
    session_write_close();
    $suggestions = $this->Item_expenses->get_search_suggestions($this->input->get('term'),100);
  ...
  */
  function suggest_category()
  {
    //allow parallel searchs to improve performance.
    session_write_close();
    $suggestions = $this->Item_expenses->get_category_suggestions($this->input->get('term'));
    echo json_encode($suggestions);
  }

  function get_info($item_id=-1)
  {
    echo json_encode($this->Item_expenses->get_info($item_id));
  }
  ...
  function view($item_id=-1,$redirect=0, $sale_or_receiving = 'sale')
  {
    ...
    if ($item_id != -1)
    {
      $data['next_item_id'] = $this->Item_expenses->get_next_id($item_id);
      $data['prev_item_id'] = $this->Item_expenses->get_prev_id($item_id);;
    }
    ...
    foreach($this->Location->get_all()->result() as $location)
    {
      ...
      {        
        ...
        $data['location_items'][$location->location_id] = $this->Item_location_expenses->get_info($item_id,$location->location_id);
        $data['location_taxes'][$location->location_id] = $this->Item_location_taxes->get_info($item_id, $location->location_id);
                
        foreach($data['tiers'] as $tier)
        {          
          $tier_prices = $this->Item_location_expenses->get_tier_price_row($tier->id,$data['item_info']->item_id, $location->location_id);
    ...
    foreach($data['tiers'] as $tier)
    {
      $tier_prices = $this->Item_expenses->get_tier_price_row($tier->id,$data['item_info']->item_id);
    ...
    $data['selected_supplier'] = $this->Item_expenses->get_info($item_id)->supplier_id;
    $this->load->view("items_expenses/form",$data);
  }
  ...
  function inventory($item_id=-1)
  {
    ...
    $this->load->view("items_expenses/inventory",$data);
  }
  ...
  function bulk_edit()
  {
    ...
    $this->load->view("items_expenses/form_bulk", $data);
  }
  ...
  function save($item_id=-1)
  {
    ...
    $cur_item_info = $this->Item_expenses->get_info($item_id);

    $redirect=$this->input->post('redirect');
    $sale_or_receiving=$this->input->post('sale_or_receiving');

    if($this->Item_expenses->save($item_data,$item_id))
    {
      ...
      if ($this->input->post('item_tier'))
      {
        foreach($this->input->post('item_tier') as $tier_id => $price_or_percent)
        {
          ...
          {        
            ...
            $this->Item_expenses->save_item_tiers($tier_data,$item_id);
          }
          else
          {
            $this->Item_expenses->delete_tier_price($tier_id, $item_id);
          }
      ...
      if ($this->input->post('locations'))
      {
        ...
        {            
          ...
          $item_location_before_save = $this->Item_location_expenses->get_info($item_id,$location_id);
          $data = array(
          ...
          $this->Item_location_expenses->save($data, $item_id,$location_id);
          ...
          if (isset($item_location_data['item_tier']))
          {
            ...
            foreach($item_location_data['item_tier'] as $tier_id => $price_or_percent)
            {
              ...
              {        
                ...
                $this->Item_location_expenses->save_item_tiers($tier_data,$item_id, $location_id);
              }
              else
              {
                $this->Item_location_expenses->delete_tier_price($tier_id, $item_id, $location_id);
              }
      ...
      if($this->input->post('del_image') && $item_id != -1)
      {
          ...
          {
          $this->Item_expenses->update_image(NULL,$item_id);
      ...
      {          
        ...
        $extension = strtolower(pathinfo($_FILES["image_id"]["name"], PATHINFO_EXTENSION));
          ...
          $this->Item_expenses->update_image($image_file_id,$item_id);
      }
    }
    else //failure
    {
  ...
  function save_inventory($item_id=-1)
  {
    ...
    $employee_id=$this->Employee->get_logged_in_employee_info()->person_id;
    $cur_item_info = $this->Item_expenses->get_info($item_id);
    $cur_item_location_info = $this->Item_location_expenses->get_info($item_id);
  ...
  //  $this->Inventory->insert($inv_data);
    ...
    if($this->Item_location_expenses->save_quantity($cur_item_location_info->quantity + $this->input->post('newquantity'),$item_id))
    {
  ...
  function bulk_update()
  {
    ...
    foreach($_POST as $key=>$value)
    {
      ...
      {
        $this->Item_location_expenses->update_multiple(array('quantity' => $value), $items_to_update,$select_inventory);
      }
    ...
    if(empty($item_data) || $this->Item_expenses->update_multiple($item_data,$items_to_update,$select_inventory))
    {
  ...
  function delete()
  {
    ...
    $select_inventory=$this->get_select_inventory();
    $total_rows= $select_inventory ? $this->Item_expenses->count_all() : count($items_to_delete);
    //clears the total inventory selection
    $this->clear_select_inventory();
    if($this->Item_expenses->delete_list($items_to_delete,$select_inventory))
    {
  ...
  function excel_export() {
    $data = $this->Item_expenses->get_all($this->Item_expenses->count_all())->result_object();
    $this->load->helper('report');
    ...
    foreach ($data as $r) 
    {
      ...
      {
        $tier_id = $tier->id;
        $tier_row = $this->Item_expenses->get_tier_price_row($tier_id, $r->item_id);
  ...
  {
    $this->check_action_permission('add_update');
    $this->load->view("items_expenses/excel_import", null);
  }
          ...
          {  
            ...
            foreach($this->Tier->get_all()->result() as $tier)
            {
              ...
              if ($tier_value)
              {
                ...
                $this->Item_expenses->save_item_tiers($tier_data,isset($item_data['item_id']) ? $item_data['item_id'] :  $item_id);
              }
              else
              {
                $this->Item_expenses->delete_tier_price($tier_id, isset($item_data['item_id'])? $item_data['item_id'] :  $item_id);  
              }
              
              $counter++;
            }
            
            $item_location_before_save = $this->Item_location_expenses->get_info($item_id,$this->Employee->get_logged_in_employee_current_location_id());
            
            $this->Item_location_expenses->save_quantity($quantity!=null ? $quantity : NULL ,isset($item_data['item_id']) ? $item_data['item_id'] :  $item_id);
