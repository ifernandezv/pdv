...
class Item_kits_expenses extends Secure_area implements iData_controller
{
  function __construct()
  {
    parent::__construct('item_kits_expenses');
  }
  ...
  function index($offset=0)
  {
    ...
    {
       redirect('item_kits_expenses/index/'.$params['offset']);
    }
    $this->check_action_permission('search');
    $config['base_url'] = site_url('item_kits_expenses/sorting');
    ...
    if ($data['search'])
    {
      $config['total_rows'] = $this->Item_kit_expenses->search_count_all($data['search']);
      $table_data = $this->Item_kit_expenses->search($data['search'],$data['per_page'],$params['offset'],$params['order_col'],$params['order_dir']);
    }
    else
    {
      $config['total_rows'] = $this->Item_kit_expenses->count_all();
      $table_data = $this->Item_kit_expenses->get_all($data['per_page'],$params['offset'],$params['order_col'],$params['order_dir']);
    }
    ...
    $this->load->view('item_kits_expenses/manage',$data);
  }
  ...
  function sorting()
  {
    ...
    if ($search)
    {
      $config['total_rows'] = $this->Item_kit_expenses->search_count_all($search);
      $table_data = $this->Item_kit_expenses->search($search,$per_page,$this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'name' ,$this->input->post('order_dir') ? $this->input->post('order_dir'): 'asc');
    }
    else
    {
      $config['total_rows'] = $this->Item_kit_expenses->count_all();
      $table_data = $this->Item_kit_expenses->get_all($per_page,$this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'name' ,$this->input->post('order_dir') ? $this->input->post('order_dir'): 'asc');
    }
    $config['base_url'] = site_url('item_kits_expenses/sorting');
  ...
  function excel_export() {
    $data = $this->Item_kit_expenses->get_all()->result_object();
  ...
  function search()
  {
    ...
    $this->session->set_userdata("item_kit_search_data",$item_kit_search_data);
    $per_page=$this->config->item('number_of_items_per_page') ? (int)$this->config->item('number_of_items_per_page') : 20;
    $search_data=$this->Item_kit_expenses->search($search,$per_page,$this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'name' ,$this->input->post('order_dir') ? $this->input->post('order_dir'): 'asc');
    $config['base_url'] = site_url('item_kits_expenses/search');
    $config['total_rows'] = $this->Item_kit_expenses->search_count_all($search);
    $config['per_page'] = $per_page ;
    $this->pagination->initialize($config);        
    $data['pagination'] = $this->pagination->create_links();
  ...
  */
  function suggest()
  {
    ...
    session_write_close();
    $suggestions = $this->Item_kit_expenses->get_search_suggestions($this->input->get('term'),100);
    echo json_encode($suggestions);
  }
  
  function check_duplicate()
  {
    echo json_encode(array('duplicate'=>$this->Item_kit_expenses->check_duplicate($this->input->post('term'))));
  ...
  function view($item_kit_id=-1,$redirect=0)
  {
    ...
    $data['controller_name']=strtolower(get_class());
    $data['item_kit_info']=$this->Item_kit_expenses->get_info($item_kit_id);
    ...
    foreach($this->Location->get_all()->result() as $location)
    {
      ...
      {        
        ...
        $data['location_item_kits'][$location->location_id] = $this->Item_kit_location_expenses->get_info($item_kit_id,$location->location_id);
        $data['location_taxes'][$location->location_id] = $this->Item_kit_location_taxes->get_info($item_kit_id, $location->location_id);
                
        foreach($data['tiers'] as $tier)
        {          
          $tier_prices = $this->Item_kit_location_expenses->get_tier_price_row($tier->id,$data['item_kit_info']->item_kit_id, $location->location_id);
    ...
    foreach($data['tiers'] as $tier)
    {
      $tier_prices = $this->Item_kit_expenses->get_tier_price_row($tier->id,$data['item_kit_info']->item_kit_id);
    ...
    $this->load->view("item_kits_expenses/form",$data);
  }
  ...
  function save($item_kit_id=-1)
  {
    ...
    if($this->Item_kit_expenses->save($item_kit_data,$item_kit_id))
    {
      ...
      if ($this->input->post('item_kit_tier'))
      {
        foreach($this->input->post('item_kit_tier') as $tier_id => $price_or_percent)
        {
          ...
          {        
            ...
            $this->Item_kit_expenses->save_item_tiers($tier_data,$item_kit_id);
          }
          else
          {
            $this->Item_expenses->delete_tier_price($tier_id, $item_kit_id);
          }
        }
      }
      ...
      if($item_kit_id==-1)
      {
        ...
        echo json_encode(array('success'=>true,'message'=>$success_message,'item_kit_id'=>$item_kit_data['item_kit_id']));
      ...
      if ($this->input->post('locations'))
      {
        ...
        {            
          ...
          $this->Item_kit_location_expenses->save($data, $item_kit_id,$location_id);
          ...
          if (isset($item_kit_location_data['item_tier']))
          {
            ...
            foreach($item_kit_location_data['item_tier'] as $tier_id => $price_or_percent)
            {
              ...
              {        
                ...
                $this->Item_kit_location_expenses->save_item_tiers($tier_data,$item_kit_id, $location_id);
              }
              else
              {
                $this->Item_kit_location_expenses->delete_tier_price($tier_id, $item_kit_id, $location_id);
              }
      ...
      if ($this->input->post('item_kit_item'))
      {
        ...
        $this->Item_kit_items_expenses->save($item_kit_items, $item_kit_id);
      }
  ...
  function delete()
  {
    ...
    if($this->Item_kit_expenses->delete_list($item_kits_to_delete))
    {
