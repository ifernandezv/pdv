...
class Sales extends Secure_area
{
  ...
  function closeregister() 
  {
    ...
    $close_register_name = $this->Register->get_info($this->Employee->get_logged_in_employee_current_register_id());
    ...
    if ($this->input->post('closing_amount') != '') {
      ...
      $this->load->view('sales/closing_amount', array(
        ...
        'closeout'=>$cash_register->open_amount + $this->Sale->get_cash_sales_total_for_shift($cash_register->shift_start, date("Y-m-d H:i:s")        ), 'employee_close' => $employee_close->first_name.' '.$employee_close->last_name.' - ('.$employee_close->username.')',
  ...
  function item_search()
  {
    ...
    session_write_close();
    $suggestions = $this->Item->get_item_search_suggestions($this->input->get('term'),20);
    $suggestions = array_merge($suggestions, $this->Item_kit->get_item_kit_search_suggestions($this->input->get('term'),20));
  ...
  function select_customer()
  {
    ...
    if ($this->Customer->exists($customer_id))
    {
      ...
      {
        $data['error']='<h1>'.lang('customers_bad_location').'</h1>';
        $customer_id = -1;
      }
  ...
  function change_mode()
  {
    ...
    if ($mode == 'store_account_payment')
    {
      ...
      $this->sale_lib->set_sold_by_employee_id(1);
    }
  ...
  {
     $this->sale_lib->set_fecha_pago($this->input->post('fecha_pago'));
  }
  ...
  {    
    ...
    if (
      ($this->input->post('payment_type') == lang('sales_store_account') && $this->sale_lib->get_customer() == -1) ||
      ($this->input->post('payment_type') == lang('sales_amortizacion') && $this->sale_lib->get_customer() == -1) ||
      ($this->input->post('payment_type') == lang('sales_cuotainicial') && $this->sale_lib->get_customer() == -1) ||  
  ...
  function edit_item($line)
  {
    ...
    $this->form_validation->set_rules('plazo', 'lang:items_plazo', 'numeric');
    $this->form_validation->set_rules('tasa', 'lang:items_tasa', 'numeric');
    $this->form_validation->set_rules('frecuencia', 'lang:items_frecuencia', 'numeric');
    $this->form_validation->set_rules('cuotainicial', 'lang:items_cuotainicial', 'numeric');
    $this->form_validation->set_rules('cuotamensual', 'lang:items_cuotamensual', 'numeric');
    ...
    $cuotainicial = $this->input->post("cuotainicial");
    $discount = $this->input->post("discount");
    
    $cuota = 0;
    $cuota = $tasa/12/100*$frecuencia;
    $cuota2 = $cuota + 1 ;
    $cuota2 = pow($cuota2,-$frecuencia);
    $cuotamensual = ($cuota * $price-$cuotainicial) / (1 - $cuota2) ;
    ...
    if ($this->form_validation->run() != FALSE)
    {
      $this->sale_lib->edit_item($line,$description,$serialnumber,$quantity,$plazo,$tasa,$frecuencia,$cuotainicial,$cuotamensual,$discount,$price);
    }
    else
    {
...
/*    if($this->sale_lib->is_kit_or_item($line) == 'item')
  ...
  function complete()
  {
    ...
    $data['latsale'] = $this->sale_lib->get_latsale();
    $date['longsale'] = $this->sale_lib->get_longsale();
  ...
  function receipt($sale_id)
  {
    ...
    $data['latsale'] = $this->Sale->get_latsale($sale_id);
    $data['longsale'] = $this->Sale->get_longsale($sale_id);
    $data['show_comment_on_receipt'] = $this->Sale->get_comment_on_receipt($sale_id);
    $data['transaction_time']= date(get_date_format().' '.get_time_format(), strtotime($sale_info['sale_time']));
    $data['fecha_pago'] = $this->Sale->get_fecha_pago($sale_id);
    $customer_id=$this->sale_lib->get_customer();
    ...
    if($customer_id!=-1)
    {
      ...
      $data['customer_id'] = $customer_id;
      $data['customer_name']=$cust_info->first_name.' '.$cust_info->last_name;
      $data['customer']=$data['customer_name'].($cust_info->company_name==''  ? '' :' - '.$cust_info->company_name).($cust_info->account_number==''  ? '' :' - '.$cust_info->account_number);
      $data['customer_address_1'] = $cust_info->address_1;
      $data['customer_address_2'] = $cust_info->address_2;
      $data['customer_city'] = $cust_info->city;
      $data['customer_state'] = $cust_info->state;
  ...
  function save($sale_id)
  {
    $sale_data = array(
      ...
      'latsale' => $this->input->post('latsale'),
      'longsale' => $this->input->post('longsale'),
  ...
  {    
    ...
    $data['comment'] = $this->sale_lib->get_comment();
    $data['latsale'] = $this->sale_lib->get_latsale();
    $data['longsale'] = $this->sale_lib->get_longsale();
    $data['show_comment_on_receipt'] = $this->sale_lib->get_comment_on_receipt();
    ...
    else
    {
      $data['payment_options']=array(
        ...
        if($this->config->item('customers_store_accounts') && $this->sale_lib->get_mode() != 'store_account_payment') 
        {
          ...
          $data['payment_options']=array_merge($data['payment_options'],  array(lang('sales_amortizacion') => lang('sales_amortizacion')    
          ));
          $data['payment_options']=array_merge($data['payment_options'],  array(lang('sales_cuotainicial') => lang('sales_cuotainicial')    
          ));
        }
    }
    ...
    $location_id_actual = $this->Employee->get_logged_in_employee_current_location_id();

  
  if ($location_id_actual == $info->location_id )
  {
      if($customer_id!=-1)
      {
        $info=$this->Customer->get_info($customer_id);
  ...
  function suspend($suspend_type = 1)
  {
    ...
    $latsale = $this->sale_lib->get_latsale();
    $longsale = $this->sale_lib->get_longsale();
    
    $show_comment_on_receipt = $this->sale_lib->get_comment_on_receipt();
    $emp_info=$this->Employee->get_info($employee_id);
    $fecha_pago = $this->sale_lib->get_fecha_pago();
    ...
    $pagos_store_account = $this->sale_lib->get_payment_amount(lang('sales_store_account'));
    
    $pagos_amortizacion = $this->sale_lib->get_payment_amount(lang('sales_amortizacion'));
    $pagos_cuotainicial = $this->sale_lib->get_payment_amount(lang('sales_cuotainicial'));
    
    //BALANCE IFERNANDEZ
    $pagos_total = $pagos_store_account + $pagos_amortizacion + $pagos_cuotainicial;
    $data['balance'] = $pagos_total;
    ...
    $sale_id = $this->Sale->save($data['cart'], $customer_id,$employee_id, $sold_by_employee_id, $comment,$latsale,$longsale, $show_comment_on_receipt,$data['payments'], $sale_id, $suspend_type,'','',$this->config->item('change_sale_date_when_suspending') ? date('Y-m-d H:i:s') : FALSE,$fecha_pago, $data['balance']);
  ...
  function set_latsale() 
  {
     $this->sale_lib->set_latsale($this->input->post('latsale'));
  }
  
  function set_longsale() 
  {
     $this->sale_lib->set_longsale($this->input->post('longsale'));
  }
