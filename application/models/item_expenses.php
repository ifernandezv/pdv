<?php
class Item_expenses extends CI_Model
{
  /*
  ...
  */
  function get_all($limit=10000, $offset=0,$col='item_id',$order='desc')
  {
    ...
    $this->db->select('items_expenses.*,
    location_items_expenses.quantity as quantity, 
    location_items_expenses.cost_price as location_cost_price,
    location_items_expenses.unit_price as location_unit_price');
    
    $this->db->from('items_expenses');
    $this->db->join('location_items_expenses', 'location_items_expenses.item_id = items_expenses.item_id and location_id = '.$current_location, 'left');
    $this->db->where('items_expenses.deleted',0);
  ...
  {
    $items_table = $this->db->dbprefix('items_expenses');
    $item_kits_table = $this->db->dbprefix('item_kits_expenses');
  ...
  function get_tier_price($tier_id,$item_id)
  {
  ...
  */
  function get_info($item_id)
  {
    ...
    if (!is_numeric($item_id))
    {
      ...
      $fields = $this->db->list_fields('items_expenses');
  ...
  */
  function save(&$item_data,$item_id=false)
  {
    ...
    {
      if($this->db->insert('items_expenses',$item_data))
      {
    ...
    $this->db->where('item_id', $item_id);
    return $this->db->update('items_expenses',$item_data);
  }
  ...
  */
  ...
  {
    $item_info = $this->Item_expenses->get_info($item_id);
    ...
    $this->db->where('item_id', $item_id);
    return $this->db->update('items_expenses', array('deleted' => 1));
  }
  ...
  */
  ...
  function search($search, $category = false, $limit=20,$offset=0,$column='name',$orderby='asc')
  {
    $current_location=$this->Employee->get_logged_in_employee_current_location_id();
      ...
      $this->db->dbprefix('items_expenses').".item_id LIKE '%".$this->db->escape_like_str($search)."%' or 
      category LIKE '%".$this->db->escape_like_str($search)."%') and deleted=0");
      
      if ($category)
      {
        $this->db->where('items_expenses.category', $category);
      }
  ...
  function update_image($file_id,$item_id)
  {
    ...
    return $this->db->update('items_expenses');
  }
