...
class Detailed_commissions extends Report
{
  ...
  public function getDataColumns()
  {
    ...
    $return['details'][] = array('data'=>lang('reports_fixed_price'), 'align'=> 'left');
  ...
  public function getData()
  {
    ...
    $this->db->select('sale_id, sale_time, sale_date, sum(quantity_purchased) as items_purchased, CONCAT(first_name," ",last_name) as customer_name, sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax, sum(profit) as profit, sum(commission_fixed) as commission, payment_type, comment', false);
    ...
    $this->db->select('sale_id, sale_time, sale_date, item_number, item_unit_price, tier_id, items.item_id , items.product_id as item_product_id,item_kits.product_id as item_kit_product_id, item_kit_number, items.name as item_name, item_kits.name as item_kit_name, sales_items_temp.category, quantity_purchased, serialnumber, items.commission_fixed as item_commission_fixed, item_kits.commission_fixed as item_kit_commission_fixed,sales_items_temp.description, subtotal,total, tax, profit,discount_percent', false);
    $this->db->from('sales_items_temp');
