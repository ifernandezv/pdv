...
class Summary_categories_customers extends Report
{
  ...
  {
    $this->db->select('category, customer_id, sum(subtotal) as subtotal, sum(total) as total, sum(profit) as profit, sum(quantity_purchased) as item_sold', false);
    ...
    if ($this->params['customer'] > 0 ){
    $this->db->where($this->db->dbprefix('sales_items_temp').'.customer_id', $this->params['customer']);
  ...
  {
    $this->db->select('sum(subtotal) as subtotal, sum(total) as total, sum(profit) as profit', false);
    $this->db->from('sales_items_temp');
    if ($this->params['sale_type'] == 'sales')
    {
      $this->db->where('quantity_purchased > 0');
