<?php
class Item_location_expenses extends CI_Model
{
  ...
  function exists($item_id,$location=false)
  {
    ...
    $this->db->from('location_items_expenses');
  ...
  function save($item_location_data,$item_id=-1,$location_id=false)
  {
    ...
    if (!$this->exists($item_id,$location_id))
    {
      ...
      return $this->db->insert('location_items_expenses',$item_location_data);
    }

    $this->db->where('item_id',$item_id);
    $this->db->where('location_id',$location_id);
    return $this->db->update('location_items_expenses',$item_location_data);
  ...
  function save_quantity($quantity, $item_id, $location_id=false)
  {
    ...
    $sql = 'INSERT INTO '.$this->db->dbprefix('location_items_expenses'). ' (quantity, item_id, location_id)'
        . ' VALUES (?, ?, ?)'
  ...
  */
  ...
  function get_info($item_id,$location=false)
  {
    ...
    else
    {
      ...
      $fields = $this->db->list_fields('location_items_expenses');
