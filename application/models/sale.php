...
class Sale extends CI_Model
{
  ...
  function get_cash_sales_total_for_shift($shift_start, $shift_end)
    ...
    foreach($sales_payments as $row)
    {
      ...
      if ($row['payment_type'] == lang('sales_cash'))
      {
        $pagos = $pagos + $row['payment_amount'];
        //echo "PAGO EFECTIVO   ".$pagos."<br>";    
      }
      if ($row['payment_type'] == lang('sales_store_account') || $row['payment_type'] == lang('sales_amortizacion') || $row['payment_type'] == 'Cuota Inicial')
      {
        if ($row['payment_amount']<0)  
          { 
            $pagos  = $pagos + ($row['payment_amount']*-1);
          //  echo "PAGO STORE   ".$pagos."<br>";
          }      
      }
    }
  ...
  //print_r($sales_payments);
    ...
    if (isset($payment_data[lang('sales_cash')]) || isset($payment_data[lang('sales_store_account')]))
    {
      if ($payment_data['sales_store_account']['payment_amount']<0)
      {
        echo $payment_data[lang('sales_cash')]['payment_amount'];
  ...
  **/  
  ...
  function save ($items,$customer_id,$employee_id, $sold_by_employee_id, $comment, $latsale, $longsale,$show_comment_on_receipt,$payments,$sale_id=false, $suspended = 0, $cc_ref_no = '', $auth_code = '', $change_sale_date=false,$fecha_pago, $balance=0, $store_account_payment = 0)
  {
    ...
    $sales_data['fecha_pago'] = date('Y-m-d H:i:s', strtotime($fecha_pago));
    ...
    if ($suspended != 2)
    {
     ...
     if ($sale_id !== FALSE)
     {
       ...
       $previous_amortizacion_amount = $this->get_amortizacion_payment_total($sale_id);
       $previous_cuotainicial_amount = $this->get_cuotainicial_payment_total($sale_id);
       $total_pagos = $previous_store_account_amount + $previous_amortizacion_amount + $previous_cuotainicial_amount;
     }
      ...
      if($customer_id > 0 && $balance)
      {
         $store_account_transaction = array(
          ...
          'location_id'=>$this->Customer->get_info($customer_id)->location_id,
            'sale_id'=>$sale_id,
          'comment'=>$comment,
            'transaction_amount'=>$balance -  $total_pagos ,
          'balance'=>$this->Customer->get_info($customer_id)->balance,
          'date' => date('Y-m-d H:i:s')
        );
        
        if ($balance - $total_pagos)
        {
       ...
       elseif ($customer_id > 0 && $total_pagos) //We had a store account payment before has one...We need to log this
       {
          $store_account_transaction = array(
             'customer_id'=>$customer_id,
          'location_id'=>$this->Customer->get_info($customer_id)->location_id,
             'sale_id'=>$sale_id,
           'comment'=>$comment,
             'transaction_amount'=>-$total_pagos,
           'balance'=>$this->Customer->get_info($customer_id)->balance,
           'date' => date('Y-m-d H:i:s')
         );
    ...
    foreach($items as $line=>$item)
    {
      if (isset($item['item_id']))
      {
        ...
        (
          ...
          'cuotamensual'=>$item['cuotamensual']?$item['cuotamensual']:0 ,
...
//  ACA ESTA EL PROBLEMA !!!!
    ...
    $this->db->or_where('payment_type',lang('sales_amortizacion'));
    $this->db->or_where('payment_type',lang('sales_cuotainicial'));
    $this->db->where('sale_id',$sale_id);
    ...
    if($to_be_paid_result->num_rows >=1)
    {
      foreach($to_be_paid_result->result() as $to_be_paid)
      {
        if($to_be_paid->payment_amount) 
        {
          ...
          echo '<script type="text/javascript">alert ("RESTANDO '.$to_be_paid->payment_amount.'")</script>'; 
                  
          }
          else
          {
            $this->db->set('balance','balance+'.$to_be_paid->payment_amount,false);
  /*        echo '<script type="text/javascript">alert ("SUMANDO '.$to_be_paid->payment_amount.'")</script>';
   */        
          }
          $this->db->where('person_id', $customer_id);
  ...
  function get_latsale($sale_id)
  {
    $this->db->from('sales');
    $this->db->where('sale_id',$sale_id);
    return $this->db->get()->row()->latsale;
  }
  
  function get_fecha_pago($sale_id)
  {
    $this->db->from('sales');
    $this->db->where('sale_id',$sale_id);
    return $this->db->get()->row()->fecha_pago;
  }
  
  function get_longsale($sale_id)
  {
    $this->db->from('sales');
    $this->db->where('sale_id',$sale_id);
    return $this->db->get()->row()->longsale;
  }
  ...
  public function create_sales_items_temp_table($params)
  {
    ...
    elseif (isset($params['start_date']) && isset($params['end_date']))
    {
      ...
      {
        $where .=' and suspended != 0 and '.$this->db->dbprefix('sales').'.store_account_payment=0';        
      }
      ...
      {
        $where .=' and suspended != 2 and '.$this->db->dbprefix('sales').'.store_account_payment=0';          
      }
    }
    elseif ($this->config->item('hide_layaways_sales_in_reports'))
    {
  ...
  function _create_sales_items_temp_table_query($where)
  {
    ...
    (SELECT ".$this->db->dbprefix('sales').".deleted as deleted, tier_id,".$this->db->dbprefix('sales').".deleted_by as deleted_by, sale_time, date(sale_time) as sale_date, ".$this->db->dbprefix('registers').'.name as register_name,'.$this->db->dbprefix('sales_items').".sale_id, comment,payment_type, customer_id, employee_id, sold_by_employee_id,
    ".$this->db->dbprefix('items').".item_id, commission_fixed, NULL as item_kit_id, supplier_id, quantity_purchased, item_cost_price, item_unit_price, category, 
    ...
    *(SUM(CASE WHEN cumulative = 1 THEN percent ELSE 0 END))/100) as tax,
    ...
    UNION ALL
    (SELECT ".$this->db->dbprefix('sales').".deleted as deleted, tier_id,".$this->db->dbprefix('sales').".deleted_by as deleted_by, sale_time, date(sale_time) as sale_date, ".$this->db->dbprefix('registers').'.name as register_name,'.$this->db->dbprefix('sales_item_kits').".sale_id, comment,payment_type, customer_id, employee_id, sold_by_employee_id,
    NULL as item_id, ".$this->db->dbprefix('item_kits').".item_kit_id, commission_fixed, '' as supplier_id, quantity_purchased, item_kit_cost_price, item_kit_unit_price, category, 
  ...
  {    
    ...
    $this->db->where_in('sales.location_id', $location_id);
  ...
  function get_amortizacion_payment_total($sale_id)
  {
    $this->db->select('SUM(payment_amount) as amortizacion_total', false);
    $this->db->from('sales_payments');
    $this->db->where('sale_id', $sale_id);
    $this->db->where('payment_type', lang('sales_amortizacion'));  
  
    $sales_payments = $this->db->get()->row_array();  
    
    return $sales_payments['amortizacion_total'] ? $sales_payments['amortizacion_total'] : 0;
  }
  
  function get_cuotainicial_payment_total($sale_id)
  {
    $this->db->select('SUM(payment_amount) as cuotainicial_total', false);
    $this->db->from('sales_payments');
    $this->db->where('sale_id', $sale_id);
    $this->db->where('payment_type', 'Cuota Inicial');  
  
    $sales_payments = $this->db->get()->row_array();  
    
    return $sales_payments['cuotainicial_total'] ? $sales_payments['cuotainicial_total'] : 0;
  }
  ...
  public function create_sales_items_temp_table_register($params)
  {
    ...
    elseif (isset($params['start_date']) && isset($params['end_date']))
    {
      $where = 'WHERE sale_time BETWEEN '.$this->db->escape($params['start_date']).' and '.$this->db->escape($params['end_date']).' and '.$this->db->dbprefix('sales').'.register_id ="'.$params['register_id'].'" and '.$this->db->dbprefix('sales').'.location_id='.$this->db->escape($location_id). (($this->config->item('hide_store_account_payments_in_reports') ) ? ' and '.$this->db->dbprefix('sales').'.store_account_payment=0' : '');
  ...
  function _create_sales_items_temp_table_query_register($where)
  {
    ...
    return $this->db->query("CREATE TEMPORARY TABLE ".$this->db->dbprefix('sales_items_temp')."
    (SELECT ".$this->db->dbprefix('sales').".deleted as deleted,".$this->db->dbprefix('sales').".deleted_by as deleted_by, sale_time, date(sale_time) as sale_date, ".$this->db->dbprefix('registers').'.name as register_name,'.$this->db->dbprefix('sales_items').".sale_id, comment,payment_type, customer_id, employee_id, sold_by_employee_id," .$this->db->dbprefix('items').".item_id, NULL as item_kit_id, supplier_id, quantity_purchased, item_cost_price, item_unit_price, category,   
    discount_percent, (item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100) as subtotal,
    ".$this->db->dbprefix('sales_items').".line as line, serialnumber, ".$this->db->dbprefix('sales_items').".description as description,
    (item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100)+(item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100)*(SUM(CASE WHEN cumulative != 1 THEN percent ELSE 0 END)/100) 
    +(((item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100)*(SUM(CASE WHEN cumulative != 1 THEN percent ELSE 0 END)/100) + (item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100))
    ...
    *(SUM(CASE WHEN cumulative = 1 THEN percent ELSE 0 END))/100) as tax,
    ...
    GROUP BY sale_id, item_id, line) 
    UNION ALL
    (SELECT ".$this->db->dbprefix('sales').".deleted as deleted,".$this->db->dbprefix('sales').".deleted_by as deleted_by, sale_time, date(sale_time) as sale_date, ".$this->db->dbprefix('registers').'.name as register_name,'.$this->db->dbprefix('sales_item_kits').".sale_id, comment,payment_type, customer_id, employee_id, sold_by_employee_id,
...
function get_sales_totaled_by_id_register($shift_start, $shift_end, $register_id)
  ...
  function get_gmapslink ($latsale,$longsale, $type)
  {
    ...
    $long1=$longsale;
    if ($latsale >=0)  {  $latnorsud='N';}
    if ($latsale<0)    {  $latnorsud='S';}
    if ($longsale>=0)  {  $longstouest='E';}
    if ($longsale<0)  {  $longstouest='W';}
  
    $latsale = abs($latsale);
    $longsale = abs($longsale);
    $latdeg = floor($latsale);
    $latmin = floor(($latsale-$latdeg)*60);
        $latsec = round(($latsale-$latdeg-$latmin/60)*1000*3600)/1000;
        $longdeg = floor($longsale);
        $longmin = floor(($longsale-$longdeg)*60);
        $longsec = floor(($longsale-$longdeg-$longmin/60)*1000*3600)/1000;
            
    $gmaps = '<a href="https://www.google.com/maps/place/'.$latdeg.'&deg;'.$latmin.'&#39;'.$latsec.'&quot;'.$latnorsud.'+'.$longdeg.'&deg;'.$longmin.'&#39;'.$longsec.'&quot;'.$longstouest."/@".$lat1.",".$long1.'" target="_blank">Ubicacion '.$type.'</a>'; 
    
    return $gmaps;
  }
      
public function create_sales_items_temp_table_categories($params)
  {
    $location_id = $this->Employee->get_logged_in_employee_current_location_id();    
    $where = '';
    ...
    $return = $this->_create_sales_items_temp_table_query_categories($where);    
    return $return;
  }
  
  function _create_sales_items_temp_table_query_categories($where)
  {
    set_time_limit(0);
