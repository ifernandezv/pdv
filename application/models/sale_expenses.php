<?php
class Sale_expenses extends CI_Model
{
  public function get_info($sale_id)
  {
    $this->db->from('sales_expenses');
  ...
  function get_cash_sales_total_for_shift($shift_start, $shift_end)
    ...
    $this->db->select('sales_payments_expenses.sale_id, sales_payments.payment_type, payment_amount, payment_id', false);
       $this->db->from('sales_payments_expenses');
       $this->db->join('sales_expenses','sales_payments_expenses.sale_id=sales_expenses.sale_id');
    $this->db->where('sales_payments_expenses.payment_date >=', $shift_start);
    $this->db->where('sales_payments_expenses.payment_date <=', $shift_end);
    $this->db->where('register_id', $register_id);
    $this->db->where($this->db->dbprefix('sales_expenses').'.deleted', 0);
  ...
  function _get_all_sale_payments($sale_ids)
  {
    ...
    if (count($sale_ids) > 0)
    {
      $this->db->select('sales_payments_expenses.*, sales.sale_time');
        $this->db->from('sales_payments_expenses');
        $this->db->join('sales_expenses', 'sales_expenses.sale_id=sales_payments_expenses.sale_id');
      $this->db->where_in('sales_payments_expenses.sale_id', $sale_ids);
      $this->db->order_by('payment_date');
  ...
  function get_sales_totaled_by_id($shift_start, $shift_end)
  {
    ...
    $this->db->select('sales_expenses.sale_id', false);
      $this->db->from('sales_expenses');
      $this->db->join('sales_payments','sales_payments_expenses.sale_id=sales_expenses.sale_id');
    $this->db->where('sales_payments_expenses.payment_date >=', $shift_start);
    $this->db->where('sales_payments_expenses.payment_date <=', $shift_end);
    $this->db->where('register_id', $register_id);
    $this->db->where($this->db->dbprefix('sales_expenses').'.deleted', 0);
    ...
    if (count($sale_ids) > 0)
    {
      $where = 'WHERE '.$this->db->dbprefix('sales_expenses').'.sale_id IN('.implode(',',$sale_ids).')';
      $this->_create_sales_items_temp_table_query_expenses($where);
      $this->db->select('sale_id, SUM(total) as total', false);
      $this->db->from('sales_items_temp_expenses');
  ...
  function update_register_log($data) {
    ...
    $this->db->where('register_id', $register_id);
    return $this->db->update('register_log_expenses', $data) ? true : false;    
  }
  function insert_register($data) {
    return $this->db->insert('register_log_expenses', $data) ? $this->db->insert_id() : false;    
  }
  ...
  function is_register_log_open()
  {
    ...
    $this->db->from('register_log_expenses');
    $this->db->where('shift_end','0000-00-00 00:00:00');
    $this->db->where('register_id',$register_id);
    $query = $this->db->get();
    if($query->num_rows())
  ...
  {
    $this->db->where('sale_id', $sale_id);
    $success = $this->db->update('sales_expenses',$sale_data);
  ...
  function save ($items,$customer_id,$employee_id, $sold_by_employee_id, $comment,$show_comment_on_receipt,$payments,$sale_id=false, $suspended = 0, $cc_ref_no = '', $auth_code = '', $change_sale_date=false,$balance=0, $store_account_payment = 0)
  {
    ...
    $this->db->query('LOCK TABLES '.$this->db->dbprefix('customers').' WRITE, '.$this->db->dbprefix('sales_expenses').' WRITE, 
    '.$this->db->dbprefix('store_accounts').' WRITE, '.$this->db->dbprefix('sales_payments_expenses').' WRITE, '.$this->db->dbprefix('sales_items_expenses').' WRITE, 
    '.$this->db->dbprefix('giftcards').' WRITE, '.$this->db->dbprefix('location_items_expenses').' WRITE, 
    '.$this->db->dbprefix('inventory').' WRITE, '.$this->db->dbprefix('sales_items_taxes').' WRITE,
    '.$this->db->dbprefix('sales_item_kits_expenses').' WRITE, '.$this->db->dbprefix('sales_item_kits_taxes').' WRITE,'.$this->db->dbprefix('people').' READ,'.$this->db->dbprefix('items_expenses').' READ
    ,'.$this->db->dbprefix('employees_locations').' READ,'.$this->db->dbprefix('locations').' READ, '.$this->db->dbprefix('items_tier_prices').' READ
    , '.$this->db->dbprefix('location_items_tier_prices').' READ, '.$this->db->dbprefix('items_taxes').' READ, '.$this->db->dbprefix('item_kits_expenses').' READ
    , '.$this->db->dbprefix('location_item_kits_expenses').' READ, '.$this->db->dbprefix('item_kit_items_expenses').' READ, '.$this->db->dbprefix('employees').' READ , '.$this->db->dbprefix('item_kits_tier_prices').' READ
    ...
    if ($sale_id)
    {
      ...
      $this->db->where('sale_id', $sale_id);
      if (!$this->db->update('sales_expenses', $sales_data))
      {
        $this->db->query("ROLLBACK");
        $this->db->query('UNLOCK TABLES');
        return -1;
    ...
    else
    {
      ...
      if (!$this->db->insert('sales_expenses',$sales_data))
      {
        $this->db->query("ROLLBACK");
        $this->db->query('UNLOCK TABLES');
        return -1;
    ...
    if ($suspended != 2)
    {
      ...
      if($customer_id > 0 && $balance)
      {
         $store_account_transaction = array(
          ...
          'location_id'=>$this->Customer->get_info($customer_id)->location_id,
            'sale_id'=>$sale_id,
          'comment'=>$comment,
    ...
    foreach($payments as $payment_id=>$payment)
    {
      ...
      if (!$this->db->insert('sales_payments_expenses',$sales_payments_data))
      {
        $this->db->query("ROLLBACK");
        $this->db->query('UNLOCK TABLES');
        return -1;
    ...
    $store_account_item_id = $this->Item_expenses->get_store_account_item_id();
    ...
    foreach($items as $line=>$item)
    {
      if (isset($item['item_id']))
      {
        ...
        $cur_item_location_info = $this->Item_location_expenses->get_info($item['item_id']);
        ...
        if (!$this->db->insert('sales_items_expenses',$sales_items_data))
        {
          $this->db->query("ROLLBACK");
          $this->db->query('UNLOCK TABLES');
          return -1;
        ...
        if ($suspended != 2)
        {
          ...
          if (!$cur_item_info->is_service)
          {
            ...
            if (!$this->Item_location_expenses->save_quantity($cur_item_location_info->quantity - $item['quantity'], $item['item_id']))
            {
              $this->db->query("ROLLBACK");
              $this->db->query('UNLOCK TABLES');
              return -1;
          ...
          {      
            ...
            $this->email->subject(lang('sales_stock_alert_item_name').$this->Item_expenses->get_info($item['item_id'])->name);
      ...
      else
      {
        ...
        $cur_item_kit_location_info = $this->Item_kit_location_expenses->get_info($item['item_kit_id']);
        ...
        foreach($this->Item_kit_items_expenses->get_info($item['item_kit_id']) as $item_kit_item)
        {
          $cur_item_info = $this->Item_expenses->get_info($item_kit_item->item_id);
          $cur_item_location_info = $this->Item_location_expenses->get_info($item_kit_item->item_id);
          ...
          if ($suspended != 2)
          {
            ...
            if (!$cur_item_info->is_service)
            {
              ...
              if (!$this->Item_location_expenses->save_quantity($cur_item_location_info->quantity - ($item['quantity'] * $item_kit_item->quantity),$item_kit_item->item_id))
              {
                $this->db->query("ROLLBACK");
                $this->db->query('UNLOCK TABLES');
                return -1;
        ...
        /**  foreach($this->Item_taxes_finder->get_info($item['item_id']) as $row)
          {
            ...
            if (!in_array($tax_name, $this->sale_expenses_lib->get_deleted_taxes()))
            {  
  ...
  function delete($sale_id, $all_data = false)
  {
    ...
    if ($suspended != 2)
    {
      $this->db->select('sales_expenses.location_id, item_id, quantity_purchased');
      $this->db->from('sales_items_expenses');
      $this->db->join('sales_expenses', 'sales_expenses.sale_id = sales_items_expenses.sale_id');
      $this->db->where('sales_items_expenses.sale_id', $sale_id);
    
      foreach($this->db->get()->result_array() as $sale_item_row)
      {
        $sale_location_id = $sale_item_row['location_id'];
        $cur_item_info = $this->Item_expenses->get_info($sale_item_row['item_id']);  
        $cur_item_location_info = $this->Item_location_expenses->get_info($sale_item_row['item_id'], $sale_location_id);
      
        $cur_item_quantity = $this->Item_location_expenses->get_location_quantity($sale_item_row['item_id'], $sale_location_id);
      
        if (!$cur_item_info->is_service)
        {
          //Update stock quantity
          $this->Item_location_expenses->save_quantity($cur_item_quantity + $sale_item_row['quantity_purchased'],$sale_item_row['item_id'], $sale_location_id);
    ...
    if ($suspended != 2)
    {    
      $this->db->select('sales_expenses.location_id, item_kit_id, quantity_purchased');
      $this->db->from('sales_item_kits_expenses');
      $this->db->join('sales_expenses', 'sales_expenses.sale_id = sales_item_kits_expenses.sale_id');
      $this->db->where('sales_item_kits_expenses.sale_id', $sale_id);
    
      foreach($this->db->get()->result_array() as $sale_item_kit_row)
      {
        foreach($this->Item_kit_items_expenses->get_info($sale_item_kit_row['item_kit_id']) as $item_kit_item)
        {
          $sale_location_id = $sale_item_kit_row['location_id'];
          $cur_item_info = $this->Item_expenses->get_info($item_kit_item->item_id);
          $cur_item_location_info = $this->Item_location_expenses->get_info($item_kit_item->item_id, $sale_location_id);

          if (!$cur_item_info->is_service)
          {
            $cur_item_location_info->quantity = $cur_item_location_info->quantity !== NULL ? $cur_item_location_info->quantity : 0;
          
            $this->Item_location_expenses->save_quantity($cur_item_location_info->quantity + ($sale_item_kit_row['quantity_purchased'] * $item_kit_item->quantity),$item_kit_item->item_id, $sale_location_id);
      ...
      if (!$all_data)
      {
        ...
        {  
          ...
          'location_id'=>$this->Customer->get_info($sale_info['customer_id'])->location_id,
              'sale_id'=>$sale_id,
    ...
    if ($all_data)
    {
      $this->db->delete('sales_payments_expenses', array('sale_id' => $sale_id)); 
      $this->db->delete('sales_items_taxes', array('sale_id' => $sale_id)); 
      $this->db->delete('sales_items_expenses', array('sale_id' => $sale_id)); 
      $this->db->delete('sales_item_kits_taxes', array('sale_id' => $sale_id)); 
      $this->db->delete('sales_item_kits_expenses', array('sale_id' => $sale_id)); 
    }

    $this->db->where('sale_id', $sale_id);
    return $this->db->update('sales_expenses', array('deleted' => 1,'deleted_by'=>$employee_id));
  }
  ...
  function undelete($sale_id)
  {
    ...
    {    
      ...
      foreach($this->db->get()->result_array() as $sale_item_row)
      {
        ...
        {
          //Update stock quantity
          $this->Item_location_expenses->save_quantity($cur_item_location_info->quantity - $sale_item_row['quantity_purchased'],$sale_item_row['item_id']);
      ...
      $this->db->select('sales_expenses.location_id, item_kit_id, quantity_purchased');
      $this->db->from('sales_item_kits_expenses');
      $this->db->join('sales_expenses', 'sales.sale_id = sales_item_kits.sale_id');
      $this->db->where('sales_item_kits_expenses.sale_id', $sale_id);
    ...
    $this->db->where('sale_id', $sale_id);
    return $this->db->update('sales_expenses', array('deleted' => 0, 'deleted_by' => NULL));
  }
  ...
  function get_sale_items_taxes($sale_id, $line = FALSE)
  {
    ...
    if ($line)
    {
      $item_where = 'and '.$this->db->dbprefix('sales_items_expenses').'.line = '.$line;
    }
    ...
    $this->db->dbprefix('sales_items_expenses'). ' USING (sale_id, item_id, line) '.
    'WHERE '.$this->db->dbprefix('sales_items_taxes').".sale_id = $sale_id".' '.$item_where.' '.
    'ORDER BY '.$this->db->dbprefix('sales_items_expenses').'.line,'.$this->db->dbprefix('sales_items_expenses').'.item_id,cumulative,name,percent');
  ...
  function get_sale_item_kits_taxes($sale_id, $line = FALSE)
  {
    ...
    if ($line)
    {
      $item_kit_where = 'and '.$this->db->dbprefix('sales_item_kits_expenses').'.line = '.$line;
    }
    ...
    $this->db->dbprefix('sales_item_kits_expenses'). ' USING (sale_id, item_kit_id, line) '.
    'WHERE '.$this->db->dbprefix('sales_item_kits_taxes').".sale_id = $sale_id".' '.$item_kit_where.' '.
    'ORDER BY '.$this->db->dbprefix('sales_item_kits_expenses').'.line,'.$this->db->dbprefix('sales_item_kits_expenses').'.item_kit_id,cumulative,name,percent');
    return $query->result_array();  
  }
  ...
  public function create_sales_items_temp_table($params)
  {
    ...
    if (isset($params['sale_ids']))
    {
      if (!empty($params['sale_ids']))
      {
        ...
        $where.='WHERE '.$this->db->dbprefix('sales_expenses').".sale_id IN(".implode(',', $params['sale_ids']).")";
      }
      else
      {
        $where.='WHERE '.$this->db->dbprefix('sales_expenses').".sale_id IN(0)";
      }
    }
    elseif (isset($params['start_date']) && isset($params['end_date']))
    {
      $where = 'WHERE sale_time BETWEEN '.$this->db->escape($params['start_date']).' and '.$this->db->escape($params['end_date']).' and '.$this->db->dbprefix('sales_expenses').'.location_id='.$this->db->escape($location_id). (($this->config->item('hide_store_account_payments_in_reports') ) ? ' and '.$this->db->dbprefix('sales_expenses').'.store_account_payment=0' : '');
    ...
    elseif ($this->config->item('hide_layaways_sales_in_reports'))
    {
      $where .='WHERE suspended = 0'.' and '.$this->db->dbprefix('sales_expenses').'.location_id='.$this->db->escape($location_id).(($this->config->item('hide_store_account_payments_in_reports') ) ? ' and '.$this->db->dbprefix('sales_expenses').'.store_account_payment=0' : '');
    }
    else
    {
      $where .='WHERE suspended != 2'.' and '.$this->db->dbprefix('sales_expenses').'.location_id='.$this->db->escape($location_id).(($this->config->item('hide_store_account_payments_in_reports') ) ? ' and '.$this->db->dbprefix('sales_expenses').'.store_account_payment=0' : '');        
    }
    ...
    $return = $this->_create_sales_items_temp_table_query_expenses($where);    
    return $return;
  }
  
  function _create_sales_items_temp_table_query_expenses($where)
  {
    set_time_limit(0);

    return $this->db->query("CREATE TEMPORARY TABLE ".$this->db->dbprefix('sales_items_temp_expenses')."
    (SELECT ".$this->db->dbprefix('sales_expenses').".deleted as deleted, tier_id,".$this->db->dbprefix('sales_expenses').".deleted_by as deleted_by, sale_time, date(sale_time) as sale_date, ".$this->db->dbprefix('registers').'.name as register_name,'.$this->db->dbprefix('sales_items_expenses').".sale_id, comment,payment_type, customer_id, employee_id, sold_by_employee_id,
    ".$this->db->dbprefix('items_expenses').".item_id, commission_fixed, NULL as item_kit_id, supplier_id, quantity_purchased, item_cost_price, item_unit_price, category, 
    discount_percent, (item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100) as subtotal,
    ".$this->db->dbprefix('sales_items_expenses').".line as line, serialnumber, ".$this->db->dbprefix('sales_items_expenses').".description as description,
    ...
    *(SUM(CASE WHEN cumulative = 1 THEN percent ELSE 0 END))/100) as tax,
    ...
    FROM ".$this->db->dbprefix('sales_items_expenses')."
    INNER JOIN ".$this->db->dbprefix('sales_expenses')." ON  ".$this->db->dbprefix('sales_items_expenses').'.sale_id='.$this->db->dbprefix('sales_expenses').'.sale_id'."
    INNER JOIN ".$this->db->dbprefix('items_expenses')." ON  ".$this->db->dbprefix('sales_items_expenses').'.item_id='.$this->db->dbprefix('items_expenses').'.item_id'."
    LEFT OUTER JOIN ".$this->db->dbprefix('suppliers')." ON  ".$this->db->dbprefix('items_expenses').'.supplier_id='.$this->db->dbprefix('suppliers').'.person_id'."
    LEFT OUTER JOIN ".$this->db->dbprefix('sales_items_taxes')." ON  "
    .$this->db->dbprefix('sales_items_expenses').'.sale_id='.$this->db->dbprefix('sales_items_taxes').'.sale_id'." and "
    .$this->db->dbprefix('sales_items_expenses').'.item_id='.$this->db->dbprefix('sales_items_taxes').'.item_id'." and "
    .$this->db->dbprefix('sales_items_expenses').'.line='.$this->db->dbprefix('sales_items_taxes').'.line'. "
    LEFT OUTER JOIN ".$this->db->dbprefix('registers')." ON  ".$this->db->dbprefix('registers').'.register_id='.$this->db->dbprefix('sales_expenses').'.register_id'."
    $where
    GROUP BY sale_id, item_id, line) 
    UNION ALL
    (SELECT ".$this->db->dbprefix('sales_expenses').".deleted as deleted, tier_id,".$this->db->dbprefix('sales_expenses').".deleted_by as deleted_by, sale_time, date(sale_time) as sale_date, ".$this->db->dbprefix('registers').'.name as register_name,'.$this->db->dbprefix('sales_item_kits_expenses').".sale_id, comment,payment_type, customer_id, employee_id, sold_by_employee_id,
    NULL as item_id, ".$this->db->dbprefix('item_kits_expenses').".item_kit_id, commission_fixed, '' as supplier_id, quantity_purchased, item_kit_cost_price, item_kit_unit_price, category, 
    ...
    ".$this->db->dbprefix('sales_item_kits_expenses').".line as line, '' as serialnumber, ".$this->db->dbprefix('sales_item_kits_expenses').".description as description,
    ...
    *(SUM(CASE WHEN cumulative = 1 THEN percent ELSE 0 END))/100) as tax,
    ...
    FROM ".$this->db->dbprefix('sales_item_kits_expenses')."
    INNER JOIN ".$this->db->dbprefix('sales_expenses')." ON  ".$this->db->dbprefix('sales_item_kits_expenses').'.sale_id='.$this->db->dbprefix('sales_expenses').'.sale_id'."
    INNER JOIN ".$this->db->dbprefix('item_kits_expenses')." ON  ".$this->db->dbprefix('sales_item_kits_expenses').'.item_kit_id='.$this->db->dbprefix('item_kits_expenses').'.item_kit_id'."
    LEFT OUTER JOIN ".$this->db->dbprefix('sales_item_kits_taxes')." ON  "
    .$this->db->dbprefix('sales_item_kits_expenses').'.sale_id='.$this->db->dbprefix('sales_item_kits_taxes').'.sale_id'." and "
    .$this->db->dbprefix('sales_item_kits_expenses').'.item_kit_id='.$this->db->dbprefix('sales_item_kits_taxes').'.item_kit_id'." and "
    .$this->db->dbprefix('sales_item_kits_expenses').'.line='.$this->db->dbprefix('sales_item_kits_taxes').'.line'. "
    LEFT OUTER JOIN ".$this->db->dbprefix('registers')." ON  ".$this->db->dbprefix('registers').'.register_id='.$this->db->dbprefix('sales_expenses').'.register_id'."
    $where
  ...
  {    
    $location_id = $this->Employee->get_logged_in_employee_current_location_id();    
    $this->db->from('sales_expenses');
    $this->db->join('customers', 'sales_expenses.customer_id = customers.person_id', 'left');
    $this->db->join('people', 'customers.person_id = people.person_id', 'left');
    $this->db->where('sales_expenses.deleted', 0);
    $this->db->where_in('suspended', $suspended_types);
    $this->db->where_in('sales_expenses.location_id', $location_id);
    ...
    for($k=0;$k<count($sales);$k++)
    {
      $item_names = array();
      $this->db->select('name');
      $this->db->from('items_expenses');
      $this->db->join('sales_items_expenses', 'sales_items_expenses.item_id = items_expenses.item_id');
      ...
      $this->db->select('name');
      $this->db->from('item_kits_expenses');
      $this->db->join('sales_item_kits_expenses', 'sales_item_kits_expenses.item_kit_id = item_kits_expenses.item_kit_id');
      $this->db->where('sale_id', $sales[$k]['sale_id']);
  ...
  function get_recent_sales_for_customer($customer_id)
  {
    ...
    $this->db->select('sales_expenses.*, SUM(quantity_purchased) as items_purchased');
    $this->db->from('sales_expenses');
    $this->db->join('sales_items_expenses', 'sales_expenses.sale_id = sales_items_expenses.sale_id');
    $this->db->where('customer_id', $customer_id);
    $this->db->where('deleted', 0);
    $this->db->order_by('sale_time DESC');
    $this->db->group_by('sales_expenses.sale_id');
    $this->db->limit(10);
  ...
  public function create_sales_items_temp_table_register($params)
  {
    $location_id = $this->Employee->get_logged_in_employee_current_location_id();    
    $where = '';
    ...
    elseif (isset($params['start_date']) && isset($params['end_date']))
    {
      $where = 'WHERE sale_time BETWEEN '.$this->db->escape($params['start_date']).' and '.$this->db->escape($params['end_date']).' and '.$this->db->dbprefix('sales_expenses').'.register_id ="'.$params['register_id'].'" and '.$this->db->dbprefix('sales_expenses').'.location_id='.$this->db->escape($location_id). (($this->config->item('hide_store_account_payments_in_reports') ) ? ' and '.$this->db->dbprefix('sales_expenses').'.store_account_payment=0' : '');
    ...
    $return = $this->_create_sales_items_temp_table_query_register($where);    
    return $return;
  }
  
  function _create_sales_items_temp_table_query_register($where)
  {
    set_time_limit(0);

    return $this->db->query("CREATE TEMPORARY TABLE ".$this->db->dbprefix('sales_items_temp_expenses')."
    (SELECT ".$this->db->dbprefix('sales_expenses').".deleted as deleted,".$this->db->dbprefix('sales_expenses').".deleted_by as deleted_by, sale_time, date(sale_time) as sale_date, ".$this->db->dbprefix('registers').'.name as register_name,'.$this->db->dbprefix('sales_items_expenses').".sale_id, comment,payment_type, customer_id, employee_id, sold_by_employee_id," .$this->db->dbprefix('items_expenses').".item_id, NULL as item_kit_id, supplier_id, quantity_purchased, item_cost_price, item_unit_price, category,   
    discount_percent, (item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100) as subtotal,
    ".$this->db->dbprefix('sales_items_expenses').".line as line, serialnumber, ".$this->db->dbprefix('sales_items_expenses').".description as description,
    (item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100)+(item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100)*(SUM(CASE WHEN cumulative != 1 THEN percent ELSE 0 END)/100) 
    +(((item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100)*(SUM(CASE WHEN cumulative != 1 THEN percent ELSE 0 END)/100) + (item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100))
    ...
    *(SUM(CASE WHEN cumulative = 1 THEN percent ELSE 0 END))/100) as tax,
    ...
    UNION ALL
    (SELECT ".$this->db->dbprefix('sales_expenses').".deleted as deleted,".$this->db->dbprefix('sales_expenses').".deleted_by as deleted_by, sale_time, date(sale_time) as sale_date, ".$this->db->dbprefix('registers').'.name as register_name,'.$this->db->dbprefix('sales_item_kits_expenses').".sale_id, comment,payment_type, customer_id, employee_id, sold_by_employee_id,
    NULL as item_id, ".$this->db->dbprefix('item_kits_expenses').".item_kit_id, '' as supplier_id, quantity_purchased, item_kit_cost_price, item_kit_unit_price, category, 
    discount_percent, (item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100) as subtotal,
    ".$this->db->dbprefix('sales_item_kits_expenses').".line as line, '' as serialnumber, ".$this->db->dbprefix('sales_item_kits_expenses').".description as description,
    (item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100)+(item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100)*(SUM(CASE WHEN cumulative != 1 THEN percent ELSE 0 END)/100) 
    +(((item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100)*(SUM(CASE WHEN cumulative != 1 THEN percent ELSE 0 END)/100) + (item_kit_unit_price*quantity_purchased-item_kit_unit_price*quantity_purchased*discount_percent/100))
  ...
  function get_debits_sales_total_for_shift($shift_start, $shift_end, $register_id)
    {
    $sales_totals = $this->get_sales_totaled_by_id_register($shift_start, $shift_end, $register_id);
    $this->db->select('sales_payments_expenses.sale_id, sales_payments.payment_type, payment_amount, payment_id', false);
       $this->db->from('sales_payments_expenses');
       $this->db->join('sales_expenses','sales_payments_expenses.sale_id=sales_expenses.sale_id');
    $this->db->where('sales_payments_expenses.payment_date >=', $shift_start);
    ...
    if (isset($payment_data[lang('sales_store_account')]))
    {
      return $payment_data[lang('sales_store_account')]['payment_amount'];
    }
    
    return 0.00;
    }    
  
  function get_pagos_sales_total_for_shift($shift_start, $shift_end, $register_id)
    {
    $sales_totals = $this->get_sales_totaled_by_id_register($shift_start, $shift_end, $register_id);
    $this->db->select('sales_payments_expenses.sale_id, sales_payments.payment_type, payment_amount, payment_id', false);
       $this->db->from('sales_payments_expenses');
...
function get_sales_totaled_by_id_register($shift_start, $shift_end, $register_id)
  {
