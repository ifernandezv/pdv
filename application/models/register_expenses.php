<?php
class Register_expenses extends CI_Model
{
  /*
  ...
  */
  ...
  {
    $this->db->from('registers_expenses');  
    ...
    else
    {
      ...
      $fields = $this->db->list_fields('registers_expenses');
  ...
  */
  function save(&$register_data,$register_id=false)
  {
    ...
    {
      if($this->db->insert('registers_expenses',$register_data))
      {
    ...
    $this->db->where('register_id', $register_id);
    return $this->db->update('registers_expenses',$register_data);
  }
  
  function delete($register_id)
  {
    $this->db->where('register_id', $register_id);
    return $this->db->update('registers_expenses', array('deleted' => 1));
  }
}
?>
