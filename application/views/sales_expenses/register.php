<div id="content-header" class="hidden-print sales_header_container">
  <h1 class="headigs"> <i class="icon fa fa-usd"></i>
    <?php echo lang('sales_expenses_register_expenses'); ?> <span id="ajax-loader"><?php echo img(array('src' => base_url().'/img/ajax-loader.gif')); ?></span>
    <?php if($this->sale_expenses_lib->get_change_sale_id()) { ?>
    <small>
      <?php echo lang('sales_expenses_editing_sale'); ?> <b> <?php echo $this->config->item('sale_prefix').' '.$this->sale_expenses_lib->get_change_sale_id(); ?> </b> 
    </small>
    <?php } ?>
  </h1>
...
<div class="clear"></div>
  ...
  <div class="row">
    <div class="sale_register_leftbox col-md-9">
      <div class="row forms-area">
        <?php if ($mode != 'store_account_payment') { ?>
            <div class="col-md-8 no-padd">
              <div class="input-append">
                <?php echo form_open("sales_expenses/add",array('id'=>'add_item_form','class'=>'form-inline', 'autocomplete'=> 'off')); ?>
                
                <?php echo form_input(array('name'=>'item','id'=>'item','class'=>'input-xlarge', 'accesskey' => 'k', 'placeholder' => lang('sales_expenses_start_typing_item_name')));
                ?>
                
                <?php echo anchor("items_expenses/view/-1/1/sale_expenses",
                  lang('sales_expenses_new_item_expenses'),
                  array('class'=>'btn btn-primary none new_item_btn','title'=>lang('sales_expenses_new_item')));
                ?>
                  
                <?php echo anchor("sales_expenses/suspended/",
                  "<div class='small_button'>".lang('sales_expenses_expenses_suspended_sales')."</div>",
                  array('class'=>'btn btn-primary none suspended_sales_btn','title'=>lang('sales_expenses_expenses_suspended_sales')));
                ?>                  
                </form>
              </div>
            </div>          
        ...
        <div class="col-md-4 no-padd">
                ...
                <?php 
        $modes_array = array();
        $modes_array['sale'] = 'Salida';
        $modes_array['return'] = 'Devolución';
        
         ?> 
                
                
          <?php echo form_open("sales_expenses/change_mode",array('id'=>'mode_form', 'autocomplete'=> 'off')); ?>
            <label ><?php echo lang('sales_expenses_mode_expenses') ?>

              <?php echo form_dropdown('mode',$modes_array,$mode,'id="mode" class=""'); ?>
            </label>
          </form>          
      </div>
      ...
      <div class="table-responsive">
        <table id="register" class="table table-bordered">
          ...
          <thead>
            <tr>
              <th ></th>
              <th class="item_name_heading" ><?php echo lang('sales_expenses_expenses_item_name_expenses'); ?></th>
              <th class="sales_item sales_items_number">
                ...
                {
                  case 'number':
                  echo lang('sales_expenses_item_number_expenses'); 
                  break;
              ...
              <th class="sales_stock"><?php echo lang('sales_expenses_stock'); ?></th>
              <th class="sales_price"><?php echo lang('sales_expenses_price'); ?></th>
              <th class="sales_quality"><?php echo lang('sales_expenses_quantity'); ?></th>
              <th ><?php echo lang('sales_expenses_total'); ?></th>
            </tr>
          </thead>
          <tbody id="cart_contents" class="sa">
            ...
            <tr class="cart_content_area">
              <td colspan='8'>
                <div class='text-center text-warning' > <h3><?php echo lang('sales_expenses_no_items_in_cart_expenses'); ?></h3></div>
              </td>
            </tr>
            <?php 
            }
            else
            {
              ...
              {
                $cur_item_location_info = isset($item['item_id']) ? $this->Item_location_expenses->get_info($item['item_id']) : $this->Item_kit_location_expenses->get_info($item['item_kit_id']);
                ?>
                <tr id="reg_item_top" bgcolor="#eeeeee" >
                  <td><?php echo anchor("sales_expenses/delete_item/$line",'<i class="fa fa-trash-o fa fa-2x text-error"></i>', array('class' => 'delete_item'));?></td>
                  <td class="text text-success"><a href="<?php echo isset($item['item_id']) ? site_url('home/view_item_modal_expenses/'.$item['item_id']) : site_url('home/view_item_kit_modal/'.$item['item_kit_id']) ; ?>" data-toggle="modal" data-target="#myModal" ><?php echo H($item['name']); ?><?php echo $item['size'] ? ' ('.H($item['size']).')': ''; ?></a></td>
                  ...
                  <td>
                    <?php
                    echo form_open("sales_expenses/edit_item/$line", array('class' => 'line_item_form', 'autocomplete'=> 'off'));    
                ...
                <tr id="reg_item_bottom">
                  <td ><?php echo lang('sales_expenses_description_abbrv').':';?></td>
                  ...
                  <td >
                    ...
                    <?php if(isset($item['is_serialized']) && $item['is_serialized']==1  && $item['name']!=lang('sales_expenses_giftcard')){
                      echo lang('sales_expenses_serial').':';
                    }?>
                  </td>
                  <td colspan="2" class="edit_serialnumber">
                    ...
                    echo form_open("sales_expenses/edit_item/$line", array('class' => 'line_item_form', 'autocomplete'=> 'off'));    
                        ...
                        if(isset($item['is_serialized']) && $item['is_serialized']==1  && $item['name']!=lang('sales_expenses_giftcard'))  {
    ...
    { /*Store Account Mode*/ ?>
      <table id="register"  class="tablesorter table table-bordered ">
        ...
        <thead>
          <tr>
            <th ><?php echo lang('sales_expenses_item_name'); ?></th>
            <th ><?php echo lang('sales_expenses_payment_amount'); ?></th>
          </tr>
        </thead>
        <tbody id="cart_contents">
          ...
          foreach(array_reverse($cart, true) as $line=>$item)  
          {
            ...
            <tr id="reg_item_top" bgcolor="#eeeeee" >
              <td class="text text-success"><a href="<?php echo site_url('items_expenses/view_modal/'.$item['item_id']) ; ?>" data-toggle="modal" data-target="#myModal" ><?php echo H($item['name']); ?></a></td>
              <td>
                <?php
                echo form_open("sales_expenses/edit_item/$line", array('class' => 'line_item_form', 'autocomplete'=> 'off'));   
    ...
    <ul class="list-inline pull-left">
      ...
      <li>
        <?php echo anchor(site_url('sales_expenses/closeregister?continue=home'), lang('sales_expenses_close_register'),array('class'=>'btn btn-primary')); ?>
      </li>
      <?php } ?>
      
      <?php
      if ($this->Register_expenses->count_all($this->Employee->get_logged_in_employee_current_location_id()) > 1)
      {
      ?>
        <li>
          <?php echo anchor(site_url('sales_expenses/clear_register'), lang('sales_expenses_change_register'),array('class'=>'btn btn-primary')); ?>
        </li>
      <?php 
      } 
      ?>
      <li> 
            ...
            <div class="row hidden-xs">
              <h1><?php echo lang('sales_expenses_recent_sales').' '.H($customer);?></h1>  
              <table id="recent_sales" class="table">
                <tr>
                  ...
                  <th align="center"><?php echo lang('sales_expenses_receipt');?></th>
                </tr>
                ...
                <?php foreach($recent_sales as $sale) {?>
                  <tr>
                    ...
                    <td align="center"><?php echo anchor('sales_expenses/receipt/'.$sale['sale_id'], lang('sales_expenses_receipt'), array('target' =>'_blank')); ?></td>
                  </tr>
                <?php } ?>
              </table>
            </div>
          ...
          <ul class="list-group">
            <li class="list-group-item nopadding">
              ...
              <div <?php if(count($cart) > 0){ echo "class='sale_form_main'";}?>>
                <?php if(count($cart) > 0){ ?>
                <?php echo form_open("sales_expenses/cancel_sale",array('id'=>'cancel_sale_form', 'autocomplete'=> 'off')); ?>
                <?php if ($mode != 'store_account_payment') { ?>
                  <input type="button" class="btn btn-warning warning-buttons" id="suspend_sale_button" value="<?php echo lang('sales_expenses_suspend_sale');?>" />
                  <input type="button" class="btn btn-warning warning-buttons" id="layaway_sale_button" style="display: none;" value="<?php echo lang('sales_expenses_layaway');?>" />
                  <input type="button" class="btn btn-warning warning-buttons" id="estimate_sale_button" style="display: none;" value="<?php echo lang('sales_expenses_estimate');?>" />
                <?php } ?>
                <input type="button" class="btn btn-danger button_dangers" id="cancel_sale_button" value="<?php echo lang('sales_expenses_cancel_sale');?>" />
              </form>
              <?php } ?>
            </div>
          </li>
          <li class="list-group-item item_tier">
            ...
            <h5 class="customer-basic-information"><?php if(isset($customer)) 
              ...
              { 
                ...
                {
                  echo lang('sales_expenses_select_customer_required');                  
                }
                else
                {
                  echo lang('sales_expenses_select_customer');
                }
              } 
              ?>
              </h5>
 ...
 <div id="customer-info" class=" <?php echo $full_width_col?>">
                <div class="clear">
                  <ul class="list-unstyled">
                    <li id="customer_view_modal"><strong><a href="<?php echo isset($customer_id) ? site_url('customers/view_modal/'.$customer_id) : site_url('customers/view_modal/'.$customer_id) ; ?>" data-toggle="modal" data-target="#myModal" ><?php echo character_limiter(H($customer), 25); ?></a></strong>                                        
                                    ...
                                    <?php  if ($customer_type == 0) {
                  $style = "style='background:red; color:white; text-align:center'";  
                  ?>
                                        <li <?php echo $style;?>>
                                           <h2>Cliente Malo</h2>
                                        </li>
                    ...
                    <?php if(!empty($customer_email)) {
                      ...
                      echo form_checkbox(array(
                        ...
                        'checked'     => 'checked',
                        )).'&nbsp;'.lang('sales_expenses_email_receipt').': <i>'.character_limiter(H($customer_email), 30).'</i>';
                      echo "</li>";
                    } ?>
                  </ul>
                </div>
            <?php                    
              ...
              echo ''.anchor("sales_expenses/delete_customer", lang('sales_expenses_detach'),array('id' => 'delete_customer','class'=>'btn-sm btn-warning'));
          ...
          <?php
              ...
              { ?>
            <?php echo form_open("sales_expenses/select_customer",array('id'=>'select_customer_form', 'autocomplete'=> 'off')); ?>
            <?php echo form_input(array('name'=>'customer','id'=>'customer','size'=>'30','value'=>lang('sales_expenses_start_typing_customer_name'), 'placeholder'=>lang('sales_expenses_start_typing_customer_name'),  'accesskey' => 'c'));?>
          </form>
          <div id="add_customer_info">
              <div id="common_or" class="common_or">
                <?php echo lang('common_or'); ?>
                <?php 
                echo anchor("customers/view/-1/3",
                  "<div class='small_button'> <span>".lang('sales_expenses_new_customer_expenses')."</span> </div>", array('class'=>'btn btn-primary none','title'=>lang('sales_expenses_new_customer_expenses'), 'id' => 'new-customer'));
                ?>
              </div>
          </div>
            ...
            if (count($tiers) > 1) 
            {
              echo "<div class=\"tiers_main clear\" style=\"display:none\">";
        ...
        <li class="list-group-item nopadding">
          ...
          <div id='sale_details'>
            <table id="sales_items" class="table">
              <tr class="warning">
                <td class="left"><?php echo lang('sales_expenses_items_in_cart'); ?>:</td>
                <td class="right"><?php echo $items_in_cart; ?></td>
              </tr>
              <?php foreach($payments as $payment) {?>
              <?php if (strpos($payment['payment_type'], lang('sales_expenses_giftcard'))!== FALSE) {?>
              <tr class="error">
                <td class="left"><?php echo $payment['payment_type']. ' '.lang('sales_expenses_balance') ?>:</td>
              ...
              <?php }?>
              <tr class="info">
                <td class="left"><?php echo lang('sales_expenses_sub_total'); ?>:</td>
              ...
              <tr class="color1">
                <td class="left">
                  ...
                  <?php if (!$is_tax_inclusive && $this->Employee->has_module_action_permission('sales', 'delete_taxes', $this->Employee->get_logged_in_employee_info()->person_id)){ ?>
                    <?php echo anchor("sales_expenses/delete_tax/".rawurlencode($name),'['.lang('common_delete').']', array('class' => 'delete_tax'));?></span>
                  <?php } ?>
                  <?php echo $name; ?>:</td>
              ...
              <?php }; ?>
              <tr class="success">
                <td ><h3 class="sales_totals"><?php echo lang('sales_expenses_total'); ?>:</h3></td>
            ...
            ?>
              <table id="register" class="table">
                <thead>
                  <tr>
                    <th id="pt_delete"></th>
                    <th id="pt_type"><?php echo lang('sales_expenses_type'); ?></th>
                    <th id="pt_amount"><?php echo lang('sales_expenses_amount'); ?></th>
                ...
                <tbody id="payment_contents">
                  ...
                  foreach($payments as $payment_id=>$payment)
                  {
                    ?>
                    <tr class="warning">
                      <td id="pt_delete"><?php echo anchor("sales_expenses/delete_payment/$payment_id",'['.lang('common_delete').']', array('class' => 'delete_payment'));?></td>
              ...
              <table id="amount_due" class="table">
                <tr class="<?php if($payments_cover_total) { echo 'success'; } else { echo 'error'; }?>">
                  <td>
                    <h4 class="sales_amount_due"><?php echo lang('sales_expenses_amount_due'); ?>:</h4>
                  </td>
                  <td>
              ...
              <div id="make_payment">
                <?php echo form_open("sales_expenses/add_payment",array('id'=>'add_payment_form', 'autocomplete'=> 'off')); ?>
                <table id="make_payment_table" class="table">
                  <tr id="mpt_top">
                    <td id="add_payment_text">
                      <label accesskey="y" for="payment_types"><?php echo lang('sales_expenses_add_payment'); ?>:</label>
                    </td>
                    <td>
                  ...
                  <tr id="mpt_bottom" >
                    <td id="tender" colspan="2">
                      <div class="input-append">
                        ...
                        <input type="button" class="btn btn-primary" id="add_payment_button" value="<?php echo lang('sales_expenses_add_payment'); ?>" />
                      </div>
        ...
        <li class="list-group-item">
          ...
          echo lang('sales_expenses_comments_receipt');
          echo form_checkbox(array(
          ...
          if((count($payments) > 0 )){?>
          <div id="finish_sale">
            <?php echo form_open("sales_expenses/complete",array('id'=>'finish_sale_form', 'autocomplete'=> 'off')); ?>
            <?php               
            if ($payments_cover_total && $customer_required_check)
            {
              echo "<input type='button' class='btn btn-success btn-large btn-block' id='finish_sale_button' value='".lang('sales_expenses_complete_sale')."' />";
            }
            ?>
          </div>
        </form>
        ...
        <div id="finish_sale">
          <?php echo form_open("sales_expenses/start_cc_processing",array('id'=>'finish_sale_form', 'autocomplete'=> 'off')); ?>
          <?php               
          if ($payments_cover_total && $customer_required_check || (is_sale_integrated_cc_processing()))
          {
            echo "<input type='button' class='btn btn-success btn-large btn-block' id='finish_sale_button' value='".lang('sales_expenses_process_credit_card')."' />";
            ...
            if (is_sale_integrated_cc_processing())
            {
              if (isset($customer) && $customer_cc_token && $customer_cc_preview)
              {
                ...
                echo lang('sales_expenses_use_saved_cc_info'). ' '.$customer_cc_preview;
                echo form_checkbox(array(
              ...
              elseif(isset($customer))
              {
                ...
                echo lang('sales_expenses_save_credit_card_info');
                echo form_checkbox(array(
      ...
      ?>
      <?php
      if($this->sale_expenses_lib->get_change_sale_id()) {
        echo '<br />';
        echo '<label id="comment_label" for="change_sale_date_enable" class="checkbox">';
        echo lang('sales_expenses_change_date');
        echo form_checkbox(array(
...
<div id="keyboardhelp" style="display: none;" title="<?php echo lang('sales_expenses_keyboard_help_title');?>">
  <p>
  <div>
    <span>[ESC] => <?php echo lang('sales_expenses_set_focus_cancel')?>[Enter] <?php echo lang('sales_expenses_will_trigger');?></span><br>
    <span>[F7]  => <?php echo lang('sales_expenses_set_focus_payment');?> [ALT][Down Arrow] <?php echo lang('sales_expenses_will_trigger_dropdown');?>, <?php echo lang('sales_expenses_and'); ?> [TAB] <?php echo lang('sales_expenses_will_focus_amount');?>.</span><br>
    <span>[F4]  => <?php echo lang('sales_expenses_set_focus_complete');?>. [Enter] <?php echo lang('sales_expenses_will_trigger');?></span>
    <br><br>
    <span> <?php echo lang('sales_expenses_in_addtion_shortcut_keys');?> [SHIFT][ALT] <?php echo lang('sales_expenses_on_the_keyboard'); ?>. <?php echo lang('sales_expenses_you_will_see_letters');?>.</span><br>
    <span>[SHIFT][ALT][o] <?php echo lang('sales_expenses_set_focus_comments');?>.</span>
    <span>[SHIFT][ALT][p] <?php echo lang('sales_expenses_set_focus_amount');?>.</span>
...
<?php if ($this->config->item('select_sales_person_during_sale')) {?>
  ...
  <div class="col-md-3">
    ...
    <?php echo lang('sales_expenses_sales_person'); ?>: 
...
<?php
$reg_info = $this->Register_expenses->get_info($this->Employee->get_logged_in_employee_current_register_id());
...
<div class="col-md-3">
  <div style="margin-left: 10px;margin-top: 20px;" id="switch_register_container">
    <?php echo lang('locations_register_name');?>: <?php echo anchor('sales_expenses/clear_register', $reg_name);?>
  </div>
  </div>
<?php
}
...
<div class="col-md-3">
  <a href="#" class="pull-right visible-lg" id="opener"><?php echo lang('sales_expenses_keyboard_help_title');?></a>
...
$(document).keydown(function(event)
{
  ...
  $(document).ready(function()
  {
    ...
    $( "#item" ).autocomplete({
      source: '<?php echo site_url("sales_expenses/item_search"); ?>',
      delay: 300,
      autoFocus: false,
      minLength: 1,
    ...
    $( "#customer" ).autocomplete({
      source: '<?php echo site_url("sales_expenses/customer_search"); ?>',
      delay: 300,
      autoFocus: false,
      minLength: 1,
      select: function(event, ui)
    ...
    {
      $(this).attr('value',<?php echo json_encode(lang('sales_expenses_start_typing_customer_name')); ?>);
    });
    
    $('#item').blur(function()
    {
      $(this).attr('value',<?php echo json_encode(lang('sales_expenses_start_typing_item_name')); ?>);
    });
    
    //Datepicker change
    $('#change_sale_date_picker').datepicker().on('changeDate', function(ev) {
      $.post('<?php echo site_url("sales_expenses/set_change_sale_date");?>', {change_sale_date: $('#change_sale_date').val()});      
    });
    ...
    {
      $.post('<?php echo site_url("sales_expenses/set_change_sale_date_enable");?>', {change_sale_date_enable: $('#change_sale_date_enable').is(':checked') ? '1' : '0'});
    });

    $('#comment').change(function() 
    {
      $.post('<?php echo site_url("sales_expenses/set_comment");?>', {comment: $('#comment').val()});
    });
            
    $('#show_comment_on_receipt').change(function() 
    {
      $.post('<?php echo site_url("sales_expenses/set_comment_on_receipt");?>', {show_comment_on_receipt:$('#show_comment_on_receipt').is(':checked') ? '1' : '0'});
    });

    $('#email_receipt').change(function() 
    {  
      $.post('<?php echo site_url("sales_expenses/set_email_receipt");?>', {email_receipt: $('#email_receipt').is(':checked') ? '1' : '0'});
    });

    $('#save_credit_card_info').change(function() 
    {
      $.post('<?php echo site_url("sales_expenses/set_save_credit_card_info");?>', {save_credit_card_info:$('#save_credit_card_info').is(':checked') ? '1' : '0'});
    });
    ...
    {
      $.post('<?php echo site_url("sales_expenses/set_use_saved_cc_info");?>', {use_saved_cc_info:$('#use_saved_cc_info').is(':checked') ? '1' : '0'});
    });
    ...
    $("#finish_sale_button").click(function()
    {
      ...
      <?php if ($is_over_credit_limit) { ?>
        if (!confirm(<?php echo json_encode(lang('sales_expenses_over_credit_limit_warning')); ?>))
        {
      ...
      <?php if(!$payments_cover_total) { ?>
        ...
        if (!confirm(<?php echo json_encode(lang('sales_expenses_payment_not_cover_total_confirmation')); ?>))
        {
          //Bring back submit and unmask if fail to confirm
          $("#finish_sale_button").show();
          $("#register_container").unmask();
      ...
      <?php if (!$this->config->item('disable_confirmation_sale')) { ?>
        if (confirm(<?php echo json_encode(lang("sales_confirm_finish_sale")); ?>))
        {
          ...
          if ($("#comment").val())
          {
            $.post('<?php echo site_url("sales_expenses/set_comment");?>', {comment: $('#comment').val()}, function()
            {
      ...
      if (confirm(<?php echo json_encode(lang("sales_confirm_suspend_sale")); ?>))
      {
        $.post('<?php echo site_url("sales_expenses/set_comment");?>', {comment: $('#comment').val()}, function() {
          <?php if ($this->config->item('show_receipt_after_suspending_sale')) { ?>
            window.location = '<?php echo site_url("sales_expenses/suspend"); ?>';
            <?php }else { ?>
              $("#register_container").load('<?php echo site_url("sales_expenses/suspend"); ?>');
          <?php } ?>
        });
      }
    });
    ...
    $("#estimate_sale_button").click(function()
    {
      if (confirm(<?php echo json_encode(lang("sales_confirm_suspend_sale")); ?>))
      {
        $.post('<?php echo site_url("sales_expenses/set_comment");?>', {comment: $('#comment').val()}, function() {
          <?php if ($this->config->item('show_receipt_after_suspending_sale')) { ?>
            window.location = '<?php echo site_url("sales_expenses/suspend/2"); ?>';
          <?php }else { ?>
            $("#register_container").load('<?php echo site_url("sales_expenses/suspend/2"); ?>');
          <?php } ?>
        });
      }
    });
    ...
    {
      $.post('<?php echo site_url("sales_expenses/set_tier_id");?>', {tier_id: $(this).val()}, function()
      {
        $("#register_container").load('<?php echo site_url("sales_expenses/reload"); ?>');
      });
    });

    $("#sold_by_employee_id").change(function()
    {
      $.post('<?php echo site_url("sales_expenses/set_sold_by_employee_id");?>', {sold_by_employee_id: $(this).val()}, function()
      {
        $("#register_container").load('<?php echo site_url("sales_expenses/reload"); ?>');
      });
    });
...
{
  if ($("#payment_types").val() == <?php echo json_encode(lang('sales_expenses_giftcard')); ?>)
  {
    $("#amount_tendered").val('');
