...
<div id="receipt_wrapper" class="receipt_<?php echo $this->config->item('receipt_text_size');?>">
  <div id="receipt_header">
    ...
    <div class="pull-right"><button class="btn btn-primary text-white hidden-print" id="new_sale_button_1" onclick="window.location='<?php echo site_url('sales_expenses'); ?>'" > <?php echo lang('sales_new_sale_expenses'); ?> </button></div>
  </div>
  ...
  <th class="left_text_align" style="width:<?php echo $discount_exists ? "33%" : "49%"; ?>;"><?php echo "Detalle"; ?></th>
  ...
  <?php 
   if (!$store_account_payment && $this->Employee->has_module_action_permission('sales_expenses', 'edit_sale', $this->Employee->get_logged_in_employee_info()->person_id)){

   $edit_sale_url = (isset($sale_type) && ($sale_type == lang('sales_layaway') || $sale_type == lang('sales_estimate'))) ? 'unsuspend' : 'change_sale';
  echo form_open("sales_expenses/$edit_sale_url/".$sale_id_raw,array('id'=>'sales_change_form')); ?>
...
<br />
  ...
  <?php if (!empty($customer_email)) { ?>
    <?php echo anchor('sales_expenses/email_receipt/'.$sale_id_raw, lang('sales_email_receipt'), array('id' => 'email_receipt','class' => 'btn btn-primary hidden-print'));?>
    <br />
  <?php }?>

<button class="btn btn-primary text-white hidden-print" id="new_sale_button_2" onclick="window.location='<?php echo site_url('sales_expenses'); ?>'" > <?php echo lang('sales_new_sale'); ?> </button>
...
<?php echo (isset($show_payment_times) && $show_payment_times) ?  date(get_date_format().' '.get_time_format(), strtotime($payment['payment_date'])) : lang('sales_payment'); ?>  <?php $splitpayment=explode(':',$payment['payment_type']); echo $splitpayment[0]; ?> <?php } ?><?php echo $this->config->item('round_cash_on_sales') && $payment['payment_type'] == lang('sales_cash') ?  str_replace('&#8209;', '-', to_currency(round_to_nearest_05($payment['payment_amount']))) : str_replace('&#8209;', '-', to_currency($payment['payment_amount'])); ?>
