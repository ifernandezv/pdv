...
<div style= "display: <?php echo $mode == 'store_account_payment' ? 'none': 'block' ;?>" id="show_hide_grid_wrapper">
  ...
  <a href="javascript:void(0);" class="btn btn-primary" id="show_grid"><?php echo lang('sales_expenses_expenses_show_grid');?></a>
  <a href="javascript:void(0);" class="btn btn-primary" id="hide_grid"><?php echo lang('sales_expenses_expenses_hide_grid');?></a>  
</div>

</div>

<div id="register_container" class="sales clearfix">
  <?php $this->load->view("sales_expenses/register"); ?>
</div>
...
$(document).ready(function()
{
  ...
  {
    $.get('<?php echo site_url("sales_expenses/categories");?>', function(json)
    {
  ...
  $(document).on('click', ".pagination.categories a", function(event)
  {
    ...
    $.get('<?php echo site_url("sales_expenses/categories");?>/'+offset, function(json)
    {
      processCategoriesResult(json);
  ...
  $(document).on('click', ".pagination.items a", function(event)
  {
    ...
    $.post('<?php echo site_url("sales_expenses/items");?>/'+offset, {category: current_category}, function(json)
    {
  ...
  $('#category_item_selection_wrapper').on('click','.category_item.category', function(event)
  {
    ...
    $.post('<?php echo site_url("sales_expenses/items");?>', {category: current_category}, function(json)
    {
      processItemsResult(json);
    }, "json");
  });
  ...
  function processItemsResult(json)
  {
    ...
    var back_to_categories_button = $("<div/>").attr('id', 'back_to_categories').attr('class', 'category_item back-to-categories col-md-2 col-sm-3 col-xs-6 ').append('<p>&laquo; '+<?php echo json_encode(lang('sales_expenses_back_to_categories')); ?>+'</p>');
