<?php if (!$this->Employee->has_module_action_permission('sales', 'only_suspended_sale', $this->Employee->get_logged_in_employee_info()->person_id)){?>
    ...
    <ul class="list-inline pull-left">
      ...
      <?php } ?>
            ...
            ?>
              <table id="register" class="table">
                <thead>
                  ...
                  <!--  <th id="pt_amount"><?php // echo lang('sales_date_2'); ?></th> !-->
                  ...
                  foreach($payments as $payment_id=>$payment)
                  {
                    ...
                    <tr class="warning">
                      ...
                      <td id="pt_amount"><?php echo  to_currency($payment['payment_amount']); ?>  </td>
                                          <!--   <td id="pt_amount"><?php // echo  $payment['payment_date']; ?>  </td>   !-->
        ...
        <li class="list-group-item">
          ...
          if (!$this->Employee->has_module_action_permission('sales', 'only_suspended_sale', $this->Employee->get_logged_in_employee_info()->person_id)){
      ...
      ?>
            <div class="field_row clearfix" id="change_sale_input_fecha">
        <div class='form_field' style="padding-top:10px">
          <div id="customer_view_modal" style="margin-bottom:5px;"><strong>Fecha de pago</strong></div>
          <?php 
     $today= date('Y-m-d H:i:s');
      if ($fecha_pago==NULL || $fecha_pago=="0000-00-00 00:00:00"){$fecha_pago=$today;}?>


          <div id="fecha_pago_picker" class="input-group date datepicker" data-date="<?php echo date(get_date_format(), strtotime($fecha_pago))?>" data-date-format=<?php echo json_encode(get_js_date_format()); ?>> <span class="input-group-addon"><i class="fa fa-calendar"></i></span> <?php echo form_input(array(
            'name'=>'fecha_pago',
            'id' => 'fecha_pago',
            'size'=>'8',
            'style'=>'width:100px', 
            'value'=>date(get_date_format(), strtotime($fecha_pago)) 
            )
          );?> </div>
        </div>
      <?php
...
$(document).keydown(function(event)
{
  ...
  $(document).ready(function()
  {
    ...
    $('#fecha_pago_picker').datepicker().on('changeDate', function(ev2) {
      $.post('<?php echo site_url("sales/set_fecha_pago");?>', {fecha_pago: $('#fecha_pago').val()});      
    });
    ...
    $("#finish_sale_button").click(function()
    {
      ...
      $.post('<?php echo site_url("sales/set_latsale");?>', {latsale: $('#latsale').val()});
      $.post('<?php echo site_url("sales/set_longsale");?>', {longsale: $('#longsale').val()});
...
<?php if ((int)$longsale == 0 or (int)$latsale == 0) { ?>
...
function updateInfo (marker) {
  var markerLatLng = marker.getPosition();
   document.getElementById('latsale').setAttribute("value", markerLatLng.lat());
   document.getElementById('longsale').setAttribute("value",markerLatLng.lng()); 
}
...
function initialize() {
  ...
  lng1 = position.coords.longitude;
    ...
    map = new google.maps.Map($('#map_canvas_preventa').get(0), myOptions);
...
<div class="clear"></div>
  ...
  <div class="row">
        ...
        <div class="col-md-3 sale_register_rightbox">
          ...
          <li class="list-group-item item_tier">
            <div id="map_canvas_preventa"></div>
        </li>
        </li>
        <li class="list-group-item spacing">
        </li>
        <li class="list-group-item nopadding" style="display:none">
        ...
        <li class="list-group-item nopadding">
          ...
          <div id="Payment_Types">
            ...
            ?>
              ...
              <table style="display:none" id="amount_due" class="table">
                <tr class="<?php if($payments_cover_total) { echo 'success'; } else { echo 'error'; }?>">
                  <td>
                    <h4 class="sales_amount_due"><?php echo lang('sales_amount_due'); ?>:</h4>
                  </td>
      ...
      if($this->sale_lib->get_change_sale_id()) {
        ...
        <?php 
              ...
              <div <?php if(count($cart) > 0){ echo "class='sale_form_main'";}?>>
                <?php if(count($cart) > 0){ ?>
                <?php echo form_open("sales/cancel_sale",array('id'=>'cancel_sale_form', 'autocomplete'=> 'off')); ?>
                <input type="button" class="btn btn-warning warning-buttons" id="layaway_sale_button" value="Guardar" />
...
<?php //CAJA REGISTRADORA/// ?>
<div class="sale_register_leftbox col-md-9" style="display:none">
      <div class="row forms-area">
        <?php if ($mode != 'store_account_payment') { ?>
            <div class="col-md-8 no-padd">
              <div class="input-append">
