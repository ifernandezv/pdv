...
<div class="clear"></div>
  ...
  <div class="row" id="form">
    ...
    <div class="widget-box">
      ...
      <div class="widget-content nopadding">
        <?php echo form_open('item_kits_expenses/save/'.$item_kit_info->item_kit_id,array('id'=>'item_kit_form','class'=>'form-horizontal')); ?>
  ...
  <div class="container-fluid">
        ...
        <div class="widget-box">
          ...
          <div class="widget-content nopadding">
            <table id="item_kit_items" class="table table-bordered table-striped table-hover text-success text-center">
              ...
              <?php foreach ($this->Item_kit_items_expenses->get_info($item_kit_info->item_kit_id) as $item_kit_item) {?>
                <tr>
                  <?php
    ...
    <div class="commission-container <?php if (!($item_kit_info->commission_percent > 0) && !($item_kit_info->commission_fixed > 0)){echo 'hidden';} ?>">
      <p style="margin-top: 10px;"><?php echo lang('common_commission_help');?></p>
        ...
        <?php echo form_dropdown('commission_type', array('percent' => lang('common_percentage'), 'fixed' => lang('common_fixed_amount')), $item_kit_info->commission_fixed > 0 ? 'fixed' : 'percent');?>
        </div>
      </div>
  ...
  $( "#item" ).autocomplete({
    source: '<?php echo site_url("items_expenses/item_search"); ?>',
    delay: 300,
    autoFocus: false,
    minLength: 0,
  ...
  $(document).ready(function()
  {
    ...
    $('#item_kit_form').validate({
      ...
      {
        $.post('<?php echo site_url("item_kits_expenses/check_duplicate");?>', {term: $('#name').val()},function(data) {
  ...
  function getPrices(items, index)
  {
    ...
    {
      $.get('<?php echo site_url("items_expenses/get_info");?>'+'/'+items[index]['item_id'], {}, function(item_info)
      {
  ...
  {  
    ...
    success:function(response)
      ...
      $('#form').unmask();
      submitting = false;
      gritter(response.success ? <?php echo json_encode(lang('common_success')); ?> +' #' + response.item_kit_id : <?php echo json_encode(lang('common_error')); ?> ,response.message,response.success ? 'gritter-item-success' : 'gritter-item-error',false,false);
      ...
      {
        window.location.href = '<?php echo site_url('item_kits_expenses'); ?>'
      }
    },
    <?php if(!$item_kit_info->item_kit_id) { ?>
    resetForm: true,
