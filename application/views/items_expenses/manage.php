...
{
  <?php
  $has_cost_price_permission = $this->Employee->has_module_action_permission('items_expenses','see_cost_price', $this->Employee->get_logged_in_employee_info()->person_id);
  ...
  enable_sorting("<?php echo site_url("$controller_name/sorting"); ?>",table_columns, <?php echo $per_page; ?>, <?php echo json_encode($order_col);?>, <?php echo json_encode($order_dir);?>);
    ...
    $('#generate_barcodes').click(function()
    {
      ...
      $(this).attr('href','<?php echo site_url("items_expenses/generate_barcodes");?>/'+selected.join('~'));
    });
  ...
  $('#generate_barcode_labels').click(function()
    {
      ...
      $(this).attr('href','<?php echo site_url("items_expenses/generate_barcode_labels");?>/'+selected.join('~'));
    });
...
{  
  if (confirm(<?php echo json_encode(lang('items_select_all_message')); ?>))
  {
    ...
    $.post('<?php echo site_url("items_expenses/select_inventory");?>', {select_inventory: $('#select_inventory').val()});
  }
...
function select_inv_none()
{
  ...
  $.post('<?php echo site_url("items_expenses/clear_select_inventory");?>', {select_inventory: $('#select_inventory').val()});  
}
select_inv_none();
</script>
<div id="content-header" class="hidden-print">
  <h1 ><i class="icon fa fa-taxi"></i> <?php echo lang('module_'.$controller_name); ?></h1>
</div>
